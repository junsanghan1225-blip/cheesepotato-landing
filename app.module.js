...
import { createClient } from './vendor/supabase-js.js?v=6bc84a0c';
// TOPIK 읽기 "문제 풀이 영상" 목록. 아주 작은 파일이라(id 목록뿐) 다른
// 자료처럼 갈래를 열 때 지연 로딩하지 않고 그냥 처음부터 받는다.
import { TQ_VIDEO_IDS } from './topik-video.js?v=6bc84a0c';
...
let XLSX = null;
const needXLSX = async () => (XLSX ??= await import('./vendor/xlsx.js?v=6bc84a0c'));
// 커리큘럼. 내용과 엔진을 갈라 두면 글을 고치다 화면을 깨지 않는다.
// 갈래 목록(drawSections)·코스(drawCourses)·문제만 풀기(dqDraw) 를 열 때만
// 받는다 — 배우기 갈래 목록도 안 본 사람에게 코스 71개 레슨을 다 물릴
// 까닭이 없다. warmLearn() 이 배우기를 여는 순간 미리 불을 붙여 둔다.
let COURSES = [], coursesP = null;
const coursesNeed = () => (coursesP ??= import('./courses.js?v=6bc84a0c').then((m) => { COURSES = m.COURSES; }));
...
const glossNeed = () => (glossP ??= import('./glossary.js?v=6bc84a0c').then((m) => {
...
const grammarNeed = () => (grammarP ??= Promise.all([
  import('./grammar.js?v=6bc84a0c'), import('./grammar-en.js?v=6bc84a0c'),
]).then(([a, b]) => { GRAMMAR = a.GRAMMAR; GRAMMAR_EN = b.GRAMMAR_EN; }));
import { grammarScan } from './grammar-find.js?v=6bc84a0c';
...
const sbNeed = () => (sbDataP ??= import('./sentences.js?v=6bc84a0c').then((m) => {
...
let makeRound = null;
const needNumbers = async () => (makeRound ??= (await import('./numbers.js?v=6bc84a0c')).makeRound);
...
/* Clarity 맞춤 이벤트. analytics.js 는 "지금 어느 화면인가"만 찍는다 —
   ...
   같은 이름표를 구글 애즈에도 흘린다(ads.js). 광고를 돌리는 동안 「이
   클릭이 가입까지 갔나」를 보려면 그쪽도 같은 순간을 알아야 하는데, 부르는
   자리를 따로 두면 언젠가 한쪽에만 넣고 넘어가게 된다. 한 자리에서 둘 다
   보낸다. ads.js 가 없거나 꺼져 있으면 window.adsTrack 이 없어서 그냥
   지나간다 — 어느 쪽이 죽어도 다른 쪽은 계속 찍힌다. */
const track = (name) => {
  try { window.clarity && window.clarity('event', name); } catch (e) {}
  try { window.adsTrack && window.adsTrack(name); } catch (e) {}
};
...
const tqNeedData = () => (tqDataP ??= Promise.all([
  import('./topik.js?v=6bc84a0c'), import('./topik2.js?v=6bc84a0c'),
  import('./topik-writing.js?v=6bc84a0c'), import('./topik-listening.js?v=6bc84a0c'),
]).then(([a, b, c, d]) => {
...
const rdNeed = () => (rdP ??= Promise.all([
  import('./reading.js?v=6bc84a0c'), grammarNeed(),
]).then(([m]) => { READING = m.READING; }));

let CONVO = null, cvP = null;
const cvNeed = () => (cvP ??= import('./convo.js?v=6bc84a0c').then((m) => { CONVO = m.CONVO; }));
...
const dictLoadSenses = () => (dictSensesP ??=
  import('./glossary-senses.js?v=6bc84a0c').then((m) => m.SENSES).catch(() => ({})));
...
const dictLoadExamples = () => (dictExamplesP ??=
  import('./glossary-examples.js?v=6bc84a0c').then((m) => m.EXAMPLES).catch(() => ({})));
...
const ltCustomOverallNeed = () => (ltCustomOverallP ??= import('./leveltest-overall.js?v=6bc84a0c').then((m) => { LT_CUSTOM.overall = m.LT_CUSTOM_OVERALL; }));
const ltCustomReadingNeed = () => (ltCustomReadingP ??= import('./leveltest-reading.js?v=6bc84a0c').then((m) => { LT_CUSTOM.reading = m.LT_CUSTOM_READING; }));
const ltCustomWritingNeed = () => (ltCustomWritingP ??= import('./leveltest-writing.js?v=6bc84a0c').then((m) => { LT_CUSTOM.writing = m.LT_CUSTOM_WRITING; }));
const ltCustomListeningNeed = () => (ltCustomListeningP ??= import('./leveltest-listening.js?v=6bc84a0c').then((m) => { LT_CUSTOM.listening = m.LT_CUSTOM_LISTENING; }));
...
function hlpRenderChrome() {
  $('hlpPanel').innerHTML =
    ...
    '<form id="hlpForm" class="hlp-form">' +
      /* 학습자가 친 질문이다. 그대로 Gemini 로 나가는 글이기도 해서,
         화면 녹화에까지 남을 자리가 아니다. */
      `<input id="hlpInput" class="hlp-in" type="text" data-clarity-mask="true" autocomplete="off" autocapitalize="off" spellcheck="false" placeholder="${esc(t('예: 가지고, -느니', 'e.g. 가지고, -느니'))}">` +
