...
import { createClient } from './vendor/supabase-js.js?v=55378792';
// TOPIK 읽기 "문제 풀이 영상" 목록. 아주 작은 파일이라(id 목록뿐) 다른
// 자료처럼 갈래를 열 때 지연 로딩하지 않고 그냥 처음부터 받는다.
import { TQ_VIDEO_IDS } from './topik-video.js?v=55378792';
...
let XLSX = null;
const needXLSX = async () => (XLSX ??= await import('./vendor/xlsx.js?v=55378792'));
// 커리큘럼. 내용과 엔진을 갈라 두면 글을 고치다 화면을 깨지 않는다.
// 갈래 목록(drawSections)·코스(drawCourses)·문제만 풀기(dqDraw) 를 열 때만
// 받는다 — 배우기 갈래 목록도 안 본 사람에게 코스 71개 레슨을 다 물릴
// 까닭이 없다. warmLearn() 이 배우기를 여는 순간 미리 불을 붙여 둔다.
let COURSES = [], coursesP = null;
const coursesNeed = () => (coursesP ??= import('./courses.js?v=55378792').then((m) => { COURSES = m.COURSES; }));
...
const glossNeed = () => (glossP ??= import('./glossary.js?v=55378792').then((m) => {
...
const grammarNeed = () => (grammarP ??= Promise.all([
  import('./grammar.js?v=55378792'), import('./grammar-en.js?v=55378792'),
]).then(([a, b]) => { GRAMMAR = a.GRAMMAR; GRAMMAR_EN = b.GRAMMAR_EN; }));
import { grammarScan } from './grammar-find.js?v=55378792';
...
const sbNeed = () => (sbDataP ??= import('./sentences.js?v=55378792').then((m) => {
...
let makeRound = null;
const needNumbers = async () => (makeRound ??= (await import('./numbers.js?v=55378792')).makeRound);
...
const tqNeedData = () => (tqDataP ??= Promise.all([
  import('./topik.js?v=55378792'), import('./topik2.js?v=55378792'),
  import('./topik-writing.js?v=55378792'), import('./topik-listening.js?v=55378792'),
]).then(([a, b, c, d]) => {
...
const rdNeed = () => (rdP ??= Promise.all([
  import('./reading.js?v=55378792'), grammarNeed(),
]).then(([m]) => { READING = m.READING; }));

let CONVO = null, cvP = null;
const cvNeed = () => (cvP ??= import('./convo.js?v=55378792').then((m) => { CONVO = m.CONVO; }));
...
const dictLoadSenses = () => (dictSensesP ??=
  import('./glossary-senses.js?v=55378792').then((m) => m.SENSES).catch(() => ({})));
...
const dictLoadExamples = () => (dictExamplesP ??=
  import('./glossary-examples.js?v=55378792').then((m) => m.EXAMPLES).catch(() => ({})));
...
/* 배우기 첫 화면의 묶음. 카드 여덟 장이 한 판에 섞여 있으면 「시험 준비」와
   ...
   TOPIK 은 갈래 하나지만 첫 화면에서는 듣기·읽기·쓰기 세 장으로 편다.
   고른 영역으로 바로 들어가고(openLearnSub), 안에서 시험(I·II)과 영역을
   다시 바꿀 수 있다. 쓰기는 TOPIK II 에만 있어서 II 로 연다. */
const LEARN_GROUPS = [
  { title: { ko: '기초 다지기', en: 'Build the basics' },
    items: ['courses', 'quiz', 'sentence'] },
  { title: { ko: 'TOPIK 시험 대비', en: 'TOPIK exam prep' },
    items: [
      { go: 'topik/listening', emoji: '🎧',
        lv: { ko: 'TOPIK · 듣기', en: 'TOPIK · LISTENING' },
        title: { ko: 'TOPIK 듣기', en: 'TOPIK listening' },
        tag: { ko: '소리를 듣고 넷 중에 고르기', en: 'Listen, then pick one of four' },
        blurb: { ko: '대본은 답을 고른 뒤에 열려요. TOPIK I·II 모두.', en: 'The script opens only after you answer. TOPIK I and II.' } },
      { go: 'topik/reading', emoji: '📖',
        lv: { ko: 'TOPIK · 읽기', en: 'TOPIK · READING' },
        title: { ko: 'TOPIK 읽기', en: 'TOPIK reading' },
        tag: { ko: '시험과 같은 유형으로', en: 'The same question types as the exam' },
        blurb: { ko: '틀리면 왜 그런지 바로 알려 줘요. 모의고사도 여기서. (기출이 아닌 창작 문항)', en: 'A wrong answer tells you why. Mock exams live here too. (Original items, not past papers.)' } },
      { go: 'topik/II/writing', emoji: '✍️',
        lv: { ko: 'TOPIK II · 쓰기', en: 'TOPIK II · WRITING' },
        title: { ko: 'TOPIK 쓰기', en: 'TOPIK writing' },
        tag: { ko: '51~54번, 문장 완성부터 긴 글까지', en: 'Q51–54, from sentence gaps to the essay' },
        blurb: { ko: '직접 쓰고 채점 기준에 맞춰 봅니다.', en: 'Write it yourself and check it against the scoring points.' } },
    ] },
...
async function drawSections() {
  ...
  $('lsecList').innerHTML = groups.map((g) =>
...
let tvP = null;
const tvNeed = () => (tvP ??= import('./travel-data.js?v=55378792').then((m) => {
...
function backToSections() {
  lsecOpen = null;
  $('learnView').classList.remove('in-sec');
  window.cpMark('learn');
...
const ltCustomOverallNeed = () => (ltCustomOverallP ??= import('./leveltest-overall.js?v=55378792').then((m) => { LT_CUSTOM.overall = m.LT_CUSTOM_OVERALL; }));
const ltCustomReadingNeed = () => (ltCustomReadingP ??= import('./leveltest-reading.js?v=55378792').then((m) => { LT_CUSTOM.reading = m.LT_CUSTOM_READING; }));
const ltCustomWritingNeed = () => (ltCustomWritingP ??= import('./leveltest-writing.js?v=55378792').then((m) => { LT_CUSTOM.writing = m.LT_CUSTOM_WRITING; }));
const ltCustomListeningNeed = () => (ltCustomListeningP ??= import('./leveltest-listening.js?v=55378792').then((m) => { LT_CUSTOM.listening = m.LT_CUSTOM_LISTENING; }));
...
$('ltDlg').addEventListener('close', () => tlStop());
