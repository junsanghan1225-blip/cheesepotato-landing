...
import { createClient } from './vendor/supabase-js.js?v=e779c374';
// TOPIK 읽기 "문제 풀이 영상" 목록. 아주 작은 파일이라(id 목록뿐) 다른
// 자료처럼 갈래를 열 때 지연 로딩하지 않고 그냥 처음부터 받는다.
import { TQ_VIDEO_IDS } from './topik-video.js?v=e779c374';
// 코스 아이콘 — 이모지 대신 선 아이콘(course-icons.js 머리말)
// 구독(Paddle) — billing.js 머리말
import { BILLING, billingLive, isPro, proInfo, loadPro, openCheckout, waitPro } from './billing.js?v=e779c374';
import { courseIcon } from './course-icons.js?v=e779c374';
...
let XLSX = null;
const needXLSX = async () => (XLSX ??= await import('./vendor/xlsx.js?v=e779c374'));
...
const coursesNeed = () => (coursesP ??= import('./courses-lite.js?v=e779c374').then((m) => { COURSES = m.COURSES; }));
let upperP = null;
const upperBlocksNeed = () => (upperP ??= coursesNeed().then(() => import('./courses-grammar-detailed.js?v=e779c374')).then((m) => {
...
const glossNeed = () => (glossP ??= import('./glossary.js?v=e779c374').then((m) => {
...
const grammarNeed = () => (grammarP ??= Promise.all([
  import('./grammar.js?v=e779c374'), import('./grammar-en.js?v=e779c374'),
]).then(([a, b]) => { GRAMMAR = a.GRAMMAR; GRAMMAR_EN = b.GRAMMAR_EN; }));
import { grammarScan } from './grammar-find.js?v=e779c374';
...
const sbNeed = () => (sbDataP ??= import('./sentences.js?v=e779c374').then((m) => {
...
let makeRound = null;
const needNumbers = async () => (makeRound ??= (await import('./numbers.js?v=e779c374')).makeRound);
...
const tqNeedData = () => (tqDataP ??= Promise.all([
  import('./topik.js?v=e779c374'), import('./topik2.js?v=e779c374'),
  import('./topik-writing.js?v=e779c374'), import('./topik-listening.js?v=e779c374'),
]).then(([a, b, c, d]) => {
...
const rdNeed = () => (rdP ??= Promise.all([
  import('./reading.js?v=e779c374'), grammarNeed(),
]).then(([m]) => { READING = m.READING; }));

let CONVO = null, cvP = null;
const cvNeed = () => (cvP ??= import('./convo.js?v=e779c374').then((m) => { CONVO = m.CONVO; }));
...
const dictLoadSenses = () => (dictSensesP ??=
  import('./glossary-senses.js?v=e779c374').then((m) => m.SENSES).catch(() => ({})));
...
const dictLoadExamples = () => (dictExamplesP ??=
  import('./glossary-examples.js?v=e779c374').then((m) => m.EXAMPLES).catch(() => ({})));
...
let tvP = null;
const tvNeed = () => (tvP ??= import('./travel-data.js?v=e779c374').then((m) => {
...
const ltCustomOverallNeed = () => (ltCustomOverallP ??= import('./leveltest-overall.js?v=e779c374').then((m) => { LT_CUSTOM.overall = m.LT_CUSTOM_OVERALL; }));
const ltCustomReadingNeed = () => (ltCustomReadingP ??= import('./leveltest-reading.js?v=e779c374').then((m) => { LT_CUSTOM.reading = m.LT_CUSTOM_READING; }));
const ltCustomWritingNeed = () => (ltCustomWritingP ??= import('./leveltest-writing.js?v=e779c374').then((m) => { LT_CUSTOM.writing = m.LT_CUSTOM_WRITING; }));
const ltCustomListeningNeed = () => (ltCustomListeningP ??= import('./leveltest-listening.js?v=e779c374').then((m) => { LT_CUSTOM.listening = m.LT_CUSTOM_LISTENING; }));
