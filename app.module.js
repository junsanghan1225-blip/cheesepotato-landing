...
import { createClient } from './vendor/supabase-js.js?v=a368d1f6';
// TOPIK 읽기 "문제 풀이 영상" 목록. 아주 작은 파일이라(id 목록뿐) 다른
// 자료처럼 갈래를 열 때 지연 로딩하지 않고 그냥 처음부터 받는다.
import { TQ_VIDEO_IDS } from './topik-video.js?v=a368d1f6';
...
let XLSX = null;
const needXLSX = async () => (XLSX ??= await import('./vendor/xlsx.js?v=a368d1f6'));
...
const coursesNeed = () => (coursesP ??= import('./courses-lite.js?v=a368d1f6').then((m) => { COURSES = m.COURSES; }));
let upperP = null;
const upperBlocksNeed = () => (upperP ??= coursesNeed().then(() => import('./courses-grammar-detailed.js?v=a368d1f6')).then((m) => {
...
const glossNeed = () => (glossP ??= import('./glossary.js?v=a368d1f6').then((m) => {
...
const grammarNeed = () => (grammarP ??= Promise.all([
  import('./grammar.js?v=a368d1f6'), import('./grammar-en.js?v=a368d1f6'),
]).then(([a, b]) => { GRAMMAR = a.GRAMMAR; GRAMMAR_EN = b.GRAMMAR_EN; }));
import { grammarScan } from './grammar-find.js?v=a368d1f6';
...
const sbNeed = () => (sbDataP ??= import('./sentences.js?v=a368d1f6').then((m) => {
...
let makeRound = null;
const needNumbers = async () => (makeRound ??= (await import('./numbers.js?v=a368d1f6')).makeRound);
...
const track = (name) => { try { window.clarity && window.clarity('event', name); } catch (e) {} };
...
const tqNeedData = () => (tqDataP ??= Promise.all([
  import('./topik.js?v=a368d1f6'), import('./topik2.js?v=a368d1f6'),
  import('./topik-writing.js?v=a368d1f6'), import('./topik-listening.js?v=a368d1f6'),
]).then(([a, b, c, d]) => {
...
const rdNeed = () => (rdP ??= Promise.all([
  import('./reading.js?v=a368d1f6'), grammarNeed(),
]).then(([m]) => { READING = m.READING; }));

let CONVO = null, cvP = null;
const cvNeed = () => (cvP ??= import('./convo.js?v=a368d1f6').then((m) => { CONVO = m.CONVO; }));
...
const dictLoadSenses = () => (dictSensesP ??=
  import('./glossary-senses.js?v=a368d1f6').then((m) => m.SENSES).catch(() => ({})));
...
const dictLoadExamples = () => (dictExamplesP ??=
  import('./glossary-examples.js?v=a368d1f6').then((m) => m.EXAMPLES).catch(() => ({})));
...
let tvP = null;
const tvNeed = () => (tvP ??= import('./travel-data.js?v=a368d1f6').then((m) => {
...
const ltCustomOverallNeed = () => (ltCustomOverallP ??= import('./leveltest-overall.js?v=a368d1f6').then((m) => { LT_CUSTOM.overall = m.LT_CUSTOM_OVERALL; }));
const ltCustomReadingNeed = () => (ltCustomReadingP ??= import('./leveltest-reading.js?v=a368d1f6').then((m) => { LT_CUSTOM.reading = m.LT_CUSTOM_READING; }));
const ltCustomWritingNeed = () => (ltCustomWritingP ??= import('./leveltest-writing.js?v=a368d1f6').then((m) => { LT_CUSTOM.writing = m.LT_CUSTOM_WRITING; }));
const ltCustomListeningNeed = () => (ltCustomListeningP ??= import('./leveltest-listening.js?v=a368d1f6').then((m) => { LT_CUSTOM.listening = m.LT_CUSTOM_LISTENING; }));
