...
import { createClient } from './vendor/supabase-js.js?v=d0eca8e9';
// TOPIK 읽기 "문제 풀이 영상" 목록. 아주 작은 파일이라(id 목록뿐) 다른
// 자료처럼 갈래를 열 때 지연 로딩하지 않고 그냥 처음부터 받는다.
import { TQ_VIDEO_IDS } from './topik-video.js?v=d0eca8e9';
// 코스 아이콘 — 이모지 대신 선 아이콘(course-icons.js 머리말)
// 구독(Paddle) — billing.js 머리말
import { BILLING, billingLive, isPro, proInfo, loadPro, openCheckout, waitPro } from './billing.js?v=d0eca8e9';
import { courseIcon } from './course-icons.js?v=d0eca8e9';
...
let XLSX = null;
const needXLSX = async () => (XLSX ??= await import('./vendor/xlsx.js?v=d0eca8e9'));
...
const coursesNeed = () => (coursesP ??= import('./courses-lite.js?v=d0eca8e9').then((m) => { COURSES = m.COURSES; }));
let upperP = null;
const upperBlocksNeed = () => (upperP ??= coursesNeed().then(() => import('./courses-grammar-detailed.js?v=d0eca8e9')).then((m) => {
...
const glossNeed = () => (glossP ??= import('./glossary.js?v=d0eca8e9').then((m) => {
...
const grammarNeed = () => (grammarP ??= Promise.all([
  import('./grammar.js?v=d0eca8e9'), import('./grammar-en.js?v=d0eca8e9'),
]).then(([a, b]) => { GRAMMAR = a.GRAMMAR; GRAMMAR_EN = b.GRAMMAR_EN; }));
import { grammarScan } from './grammar-find.js?v=d0eca8e9';
...
const sbNeed = () => (sbDataP ??= import('./sentences.js?v=d0eca8e9').then((m) => {
...
let makeRound = null;
const needNumbers = async () => (makeRound ??= (await import('./numbers.js?v=d0eca8e9')).makeRound);
...
/* Clarity 맞춤 이벤트. analytics.js 는 "지금 어느 화면인가"만 찍는다 —
   그것만으로는 사람들이 뭘 봤는지는 알아도 뭘 해냈는지는 안 보여서,
   퍼널도 스마트 이벤트도 못 만든다. 핵심 행동(레슨 완료·가입·로그인·
   단어 저장)마다 여기서 이름표를 하나씩 남긴다. */
/* 같은 이벤트를 GTM(dataLayer)에도 흘린다 — GA4 주요 이벤트나 구글 애즈 전환을
   「레벨테스트 완료」·「가입 완료」로 잡으려면 GTM 이 이것을 봐야 한다.
   GA4 이벤트 이름은 영문·숫자·밑줄만 되므로 영어 이름을 따로 둔다
...
const tqNeedData = () => (tqDataP ??= Promise.all([
  import('./topik.js?v=d0eca8e9'), import('./topik2.js?v=d0eca8e9'),
  import('./topik-writing.js?v=d0eca8e9'), import('./topik-listening.js?v=d0eca8e9'),
]).then(([a, b, c, d]) => {
...
const rdNeed = () => (rdP ??= Promise.all([
  import('./reading.js?v=d0eca8e9'), grammarNeed(),
]).then(([m]) => { READING = m.READING; }));

let CONVO = null, cvP = null;
const cvNeed = () => (cvP ??= import('./convo.js?v=d0eca8e9').then((m) => { CONVO = m.CONVO; }));
...
/* 레슨 밖으로 나가면 복습 챌린지(1분 타이머)를 꺼 둔다. 안 그러면 다른
   ...
   레슨의 lsChallengeLeft 를 깎고 있다 — 나갔다 돌아오면 이미 0초. */
function lsLeave() {
...
function open(view) {
  ...
  $('topikHdBtn').classList.remove('on');
  $('myHdBtn').classList.remove('on');
...
/* 왜 실패했는지가 화면에서 사라지면 안 된다. 「로그인에 실패했어요」만
   보이면 쓰는 사람도 고치는 사람도 다음에 무엇을 할지 알 수 없다 —
   실제로 그 한 줄 때문에 무슨 일이 났는지 못 짚었다.
   그래서 아는 까닭은 그대로 말해 주고, 모르는 까닭은 콘솔에 원문을
   남기고 화면에는 짧은 실마리(상태 코드·원문 첫 줄)를 덧붙인다. */
function authWhy(error) {
  const code = error?.status || error?.code;
  const raw = (error?.message || '').trim();
  console.error('[auth]', code ?? '', raw, error);
  if (!raw) return code ? ` (${code})` : '';
  return ` (${code ? code + ' · ' : ''}${raw.slice(0, 80)})`;
}
...
function loginMsg(error) {
  ...
  /* 망이 끊겼거나 서버에 못 닿은 것. 「비밀번호가 틀렸나」 하고 몇 번씩
     다시 치게 두지 않는다. */
  if (m.includes('failed to fetch') || m.includes('networkerror') || m.includes('load failed'))
    return t('서버에 닿지 못했어요. 인터넷 연결을 확인해 주세요.',
...
function signUpMsg(error) {
  ...
  if (m.includes('failed to fetch') || m.includes('networkerror') || m.includes('load failed'))
    return t('서버에 닿지 못했어요. 인터넷 연결을 확인해 주세요.',
             'Could not reach the server. Check your connection.');
  /* 「Database error saving new user」 — 가입 자체는 됐는데 뒤에 딸린
     트리거나 기본 행 만들기가 엎어진 것이다. 쓰는 사람이 다시 눌러도
     안 되는 종류라 그렇게 말해 준다. */
  if (m.includes('database error'))
    return t('서버에서 계정을 만들다 막혔어요. 관리자에게 알려 주세요.',
             'The server failed while creating the account. Please report this.')
         + authWhy(error);
...
const dictLoadSenses = () => (dictSensesP ??=
  import('./glossary-senses.js?v=d0eca8e9').then((m) => m.SENSES).catch(() => ({})));
...
const dictLoadExamples = () => (dictExamplesP ??=
  import('./glossary-examples.js?v=d0eca8e9').then((m) => m.EXAMPLES).catch(() => ({})));
...
/* 낱말·예문 옆 🔊 단추 아이콘. 이모지(🔊)는 기기마다 다르게 그려지고
   대개 크고 못생기다(검은 사각 배경이 박힌 것처럼 보이는 기기도 있다) —
   currentColor SVG 를 쓰면 버튼 색을 그대로 물려받아 어느 기기·테마에서도
   같은 크기·같은 색으로 나온다. */
const DICT_SAY_ICON = '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true">' +
  '<path d="M4 9v6h4l5 5V4L8 9H4z" fill="currentColor"/>' +
  '<path d="M16.3 8.5a5 5 0 0 1 0 7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>' +
  '<path d="M19.3 6a9 9 0 0 1 0 12" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" opacity=".55"/>' +
  '</svg>';
...
async function dictDrawMore(head) {
  ...
  const exHtml = ex
    ? `<p class="dict-ex"><span class="dict-ex-ko">${esc(ex.ex)}` +
      `<button class="dict-say" type="button" aria-label="${esc(t('예문 듣기', 'Play example'))}" data-say="${esc(ex.ex)}" data-audio="${AUDIO_BASE}dict/${esc(audioSlug(head))}-ex.mp3">${DICT_SAY_ICON}</button>` +
      `</span><span class="dict-ex-en">${esc(ex.en)}</span></p>`
    : '';
    ...
    el.innerHTML =
      ...
      '<div class="wb-main">' +
        `<div class="wb-word">${esc(v.head)}` +
          `<button class="dict-say" type="button" aria-label="${esc(t('발음 듣기', 'Play pronunciation'))}" data-say="${esc(v.head)}" data-audio="${AUDIO_BASE}dict/${esc(audioSlug(v.head))}.mp3">${DICT_SAY_ICON}</button>` +
        '</div>' +
        (g.meaning
...
$('dictList').addEventListener('click', (ev) => {
  ...
  if (s) { say(s.dataset.say, s.dataset.audio); return; }
...
$('wbGoogle').addEventListener('click', async () => {
  ...
  try { sessionStorage.setItem('cp_login_pending', '1'); } catch (e) {}
  ...
  if (error) {
    ...
    try { sessionStorage.removeItem('cp_login_pending'); } catch (e) {}
...
sb.auth.onAuthStateChange((_event, session) => {
  ...
  /* SIGNED_IN 은 진짜 "지금 로그인했다"(비번 로그인·구글 로그인 둘 다
     여기로 온다) 는 순간에만 뜬다 — 이미 로그인해 둔 채로 새로고침한
     INITIAL_SESSION 이나 TOKEN_REFRESHED 는 안 걸린다. 방금 가입해서
     바로 세션이 온 경우도 SIGNED_IN 이 뜨는데, 그건 위 wbForm 제출
     쪽의 '가입완료' 와 겹쳐 찍혀도 괜찮다 — 가입도 했고 로그인도 된
     게 사실이다. */
  /* 그런데 supabase-js 는 저장된 세션을 되살릴 때(새로고침·새 탭·탭으로 돌아올 때)도
     SIGNED_IN 을 낸다 — GTM 미리보기에서 쪽을 열 때마다 login 이 찍혔다. 그래서
     로그인 단추(비번·구글)를 누를 때 표시를 남기고, 그 표시가 있을 때만 센다.
     구글은 다녀오면 쪽이 새로 뜨지만 같은 탭이라 sessionStorage 가 남는다. */
  if (_event === 'SIGNED_IN') {
    let pending = false;
    try { pending = sessionStorage.getItem('cp_login_pending') === '1'; sessionStorage.removeItem('cp_login_pending'); } catch (e) {}
    if (pending) track('로그인');
  }
...
async function loadProgress() {
  doneSet = new Set(myRead('cp_done_local', [])); doneDays = [];
  const { data: { session } } = await sb.auth.getSession();
  if (!session) return;
  try {
...
const LEARN_SECTIONS = [
  {
    ...
    title: { ko: '내 코스',          en: 'My course' },
    tag:   { ko: '내 레벨에 맞춘 길과 오늘 할 레슨', en: 'Your path for your level, and today’s lesson' },
    blurb: { ko: '레벨테스트 결과로 어디서 시작할지, 다음에 무엇을 할지 정해 줍니다.',
             en: 'Your level test decides where you start and what comes next.' },
  },
  {
  ...
  },
  {
    ...
    lv:    { ko: '노트',            en: 'NOTES' },
    title: { ko: '내 노트',         en: 'My notes' },
    tag:   { ko: '공부하다 적어 두는 나만의 자리',
             en: 'Your own place to write things down' },
  ...
  {
    id: 'travel', emoji: '✈️', ready: true, pane: 'tvWrap',
    lv:    { ko: '여행',            en: 'TRAVEL' },
    title: { ko: '여행 한국어',      en: 'Travel Korean' },
    tag:   { ko: '공항·교통·숙소·식당·쇼핑 3단계 실전 회화',
             en: 'Essential 3-tier survival phrases for Korea' },
    blurb: { ko: '쉬운 말, 보통 말, 정확한 말 3단계로 바로 써먹는 실전 여행 한국어와 필수 단어를 익힙니다.',
             en: 'Learn survival phrases across Easy, Standard, and Accurate levels with essential vocab.' },
  },
];
...
/* 배우기 첫 화면의 묶음. 카드 여덟 장이 한 판에 섞여 있으면 「시험 준비」와
   ...
   모아 둔다. 갈래(LEARN_SECTIONS)는 그대로 두고, 여기서 어느 묶음에
   어느 순서로 설지만 정한다.

   TOPIK 은 여기 없다 — 배우기와 나란한 머리띠 단추(topikBtn)로 따로 연다.
   시험 준비는 한국어를 「배우는」 길과 목적이 달라서, 한 판에 섞으면
   처음 온 사람이 코스 대신 시험 문제부터 누른다. */
const LEARN_GROUPS = [
  { title: { ko: '기초 다지기', en: 'Build the basics' },
    items: ['courses', 'quiz', 'sentence'] },
...
const BEGINNER_ROADMAP = [
  ...
  {
    id:'stage3',
    tag:{ ko:'3단계. 이어 말하기', en:'Stage 3: Connecting clauses' },
    title:{ ko:'연결과 이유 표현', en:'Connectors and reason phrases' },
    points:[
      { ko:'-고(나열/순서)와 -지만(대조)으로 복합문 만들기', en:'Compound sentences with -고 (and/then) and -지만 (but)' },
      { ko:'-아/어서 vs -(으)니까(명령·청유 제약) 이유 구분 정복', en:'Master reason connectors: -아/어서 vs -(으)니까 (command rules)' },
      { ko:'-(으)면(조건 & ㄹ 탈락) · -(으)면서(동시) · -거나(선택) · -는데(배경)', en:'-(으)면 (if & ㄹ-drop), -(으)면서 (while), -거나 (either/or), -는데 (background)' },
    ],
...
function tlOpenOne(item) {
  tqGrade = item.grade;
  const tx = tlTypeTx()[item.type];
...
/* 문제 하나만 담아 연다 — topik-reading/ 정적 쪽의 "직접 풀어보기" 가
   목록이 아니라 방금 읽은 그 문제로 바로 이어지게 하려는 것이다(쓰기
   문항을 #learn/writing/<id> 로 바로 연 것과 같은 이유). 급수도 그
   문제 것으로 맞춘다 — 안 그러면 tqOf(tqGrade) 가 다른 급수를 걸러서
   "문제가 없다"로 보일 수 있다. */
function tqOpenOne(item) {
  tqMock = false; tqStopClock();
  $('tqOmr').classList.add('hidden');
  window.cpTxtSize?.(false);
  tqGrade = item.grade;
  tqRound = [item];
  tqIdx = 0; tqScore = 0; tqWrongs = []; tqBusy = false; tqSet = item.type; tqPicks = []; tqSaved = false;
  const tx = tqTypeTx()[item.type];
  tqTitle = t(`${tqE().gradeTx(tqGrade)} · ${tx ? tx.ko : ''}`, `${tqE().gradeTx(tqGrade)} · ${tx ? tx.en : ''}`);
  $('tqWall').classList.add('hidden');
  $('tqExamBody').classList.remove('hidden');
  tqPanel('tqPlay');
  tqDraw();
    ...
    span.addEventListener('click', () => {
      if (tqUnknown.has(key)) {
        ...
        /* 표시하는 그 손짓 하나로 뜻까지 본다 — 누르는 방법은 그대로 두고
           ...
           못 풀어서 점수가 시간 관리 실력만 재게 된다. 대신 지문·문항 자체는
...
/* ══ 지문에서 바로 뜻 보기 ═══════════════════════════════════════
   ...
   말인지 몰라서 못 푸는 것은 시험 연습이 아니다. 그래서 누른 낱말의
   뜻을 그 자리에서 보여 준다. 모의고사에서도 마찬가지다 — 시간을 재는
   자리일수록 낱말 때문에 못 푸는 쪽이 더 아깝다. 지문·문항·시계·모의고사
   벽은 하나도 안 건드리고 뜻풀이만 연다.
...
function tqPick(i, btn) {
  ...
  /* 문제 풀이 영상. preload 를 걸지 않는다 — 목록을 넘겨 보기만 해도
     mp4 를 전부 받아 버리면 안 되니, 답을 고른 문항의 영상만 이때
     비로소 태그가 생기면서 받기 시작한다. */
  const videoBox = $('tqVideo');
  if (TQ_VIDEO_IDS.has(q.id)) {
    videoBox.innerHTML =
      `<p class="tq-video-h">🎬 ${esc(t('문제 풀이 영상', 'Solution video'))}</p>` +
      `<video controls preload="none" src="assets/video/topik-reading/${esc(q.id)}.mp4"></video>`;
...
/* ══ 내 메모장 ═══════════════════════════════════════════════════
   ...
   쉬면 그때 적는다(ntMemoSave). 화면을 떠날 때는 기다리지 않고 바로
   적는다 — 안 그러면 마지막 몇 글자가 날아간다. */
/* 열쇠의 sb- 는 예문 만들기(sentence board) 안에 있던 시절의 이름이다.
   갈래를 따로 뗀 뒤에도 **바꾸지 않는다** — 바꾸는 순간 이미 적어 둔
   노트를 못 찾는다. 이름값보다 남이 적어 둔 글이 먼저다. */
const NT_MEMO_KEY = 'cp-sb-memo';
const NT_MEMO_CATS_KEY = 'cp-sb-memo-cats';
/* 갈래 거르개: null = 전체, '' = 갈래 없음, 그 밖 = 그 갈래 이름 */
let ntMemo = [];        // [{id, title, body, cat, pin, at, up}]
let ntMemoCats = [];    // ['문법', '어휘', …]
let ntMemoCat = null;
let ntMemoQ = '';
...
const ntMemoStore = () => {
  try {
    localStorage.setItem(NT_MEMO_KEY, JSON.stringify(ntMemo));
    localStorage.setItem(NT_MEMO_CATS_KEY, JSON.stringify(ntMemoCats));
  } catch (e) {
    /* 저장 공간이 꽉 찼다. 조용히 삼키면 「적었는데 사라졌다」가 된다. */
    $('ntMemoSaved').textContent = t('저장 공간이 꽉 찼어요', 'Storage is full');
  }
};
ntMemoLoad();
...
/* ── 되돌리기(Ctrl/Cmd+Z) ─────────────────────────────────────
   브라우저 기본 되돌리기는 한 contenteditable 칸 안에서만 도는데, 이
   편집기는 블록을 나누거나 종류를 바꿀 때마다(그때마다 nbRender() 가
   그 자리를 통째로 새로 그린다) 그 칸이 아예 다른 DOM 요소로 바뀐다 —
   그러면 브라우저의 되돌리기 이력도 함께 끊긴다. 그래서 노트마다 스스로
   지난 모양을 몇 장 들고 있다가 Ctrl+Z·Ctrl+Shift+Z 로 오간다(브라우저
   기본 되돌리기는 여기서 막는다).

   한 글자마다 한 장씩 찍으면 되돌리기를 한 번 눌러도 겨우 한 글자만
   무른다 — 그래서 이미 있는 늦춰 쓰기(디바운스) 매듭에 얹는다. 구조가
   바뀌는 손잡이(nbSave)는 그 자리에서 바로, 타이핑은 손을 뗀 뒤(늦춰
   쓰기가 실제로 도는 순간)에 한 장 찍힌다 — 그래서 한 번 누르면 방금
   고친 문단 하나만큼 무른다. */
const NB_HIST_MAX = 60;
let nbHistNoteId = null;   // 지금 되돌리기 목록이 어느 노트 것인지
let nbHistBase = null;     // 마지막으로 챙겨 둔 모양(JSON 문자열)
let nbHistUndo = [];       // 오래된 것이 앞
let nbHistRedo = [];
const nbHistSnapshot = (m) => JSON.stringify({ title: m.title, blocks: nbBlocks(m) });
/* 노트를 열 때(또는 다른 노트로 옮길 때) 되돌리기 목록을 새로 시작한다
   — 다른 노트의 되돌리기가 섞이면 안 된다. */
function nbHistReset(m) {
  nbHistNoteId = m ? m.id : null;
  nbHistBase = m ? nbHistSnapshot(m) : null;
  nbHistUndo = [];
  nbHistRedo = [];
}
/* 자료가 바뀐 뒤 부른다. 지금 모양이 마지막으로 챙겨 둔 모양과 다르면
   그 「마지막 모양」을 되돌리기 목록에 얹고, 새 모양을 기준으로 삼는다.
   다시하기 목록은 새로 고치는 순간 버린다(어디서나 그렇다). */
function nbHistCheckpoint(m) {
  if (!m || m.id !== nbHistNoteId) return;   // nbHistReset 을 안 거쳤으면 무시(다른 노트일 수 있다)
  const now = nbHistSnapshot(m);
  if (now === nbHistBase) return;
  nbHistUndo.push(nbHistBase);
  if (nbHistUndo.length > NB_HIST_MAX) nbHistUndo.shift();
  nbHistBase = now;
  nbHistRedo = [];
}
function nbHistApply(json) {
  const m = ntMemoCur();
  if (!m) return;
  /* nbRender() 가 블록 DOM 을 통째로 새로 그리므로, 지금 포커스가 어느
     블록(또는 제목 칸)에 있었는지 미리 챙겨 뒀다가 되돌린 뒤에도 그
     자리로 커서를 되돌려 놓는다 — 안 그러면 포커스가 문서 밖으로
     빠져서, Ctrl+Z 를 한 번 더 눌러도(우리 판이 지금 포커스가 문서
     안에 있는지로 가로챌지 정하므로) 안 먹는다. */
  const activeRow = document.activeElement && document.activeElement.closest
    ? document.activeElement.closest('#ntMemoBlocks .nb-row') : null;
  const wasBlockId = activeRow ? activeRow.dataset.b : null;
  const wasTitle = document.activeElement === $('ntMemoTitle');

  const parsed = JSON.parse(json);
  m.title = parsed.title;
  m.blocks = parsed.blocks;
  m.up = Date.now();
  nbHistBase = json;
  ntMemoStore();

  if (!wasTitle) {
    const stillThere = wasBlockId && m.blocks.some((b) => b.id === wasBlockId);
    nbFocus = stillThere ? wasBlockId : (m.blocks[0] ? m.blocks[0].id : null);
    nbAtEnd = true;
  }
  ntMemoDrawDoc();   // 제목 칸도 같이 되돌아가야 하니 문서 전체를 다시 그린다
  ntMemoDrawList();
  if (wasTitle) $('ntMemoTitle').focus();
}
function nbUndo() {
  if (!nbHistUndo.length) return;
  if (!ntMemoCur()) return;
  nbHistRedo.push(nbHistBase);
  nbHistApply(nbHistUndo.pop());
}
function nbRedo() {
  if (!nbHistRedo.length) return;
  if (!ntMemoCur()) return;
  nbHistUndo.push(nbHistBase);
  nbHistApply(nbHistRedo.pop());
}
/* 노트 화면 안(제목·블록)에 포커스가 있을 때만 가로챈다 — 다른 화면의
   되돌리기(단어 고치기 따위)까지 여기서 먹어 버리면 안 된다. */
document.addEventListener('keydown', (ev) => {
  if (!(ev.metaKey || ev.ctrlKey) || ev.key.toLowerCase() !== 'z') return;
  const doc = $('ntMemoDoc');
  if (!doc || !doc.contains(document.activeElement)) return;
  ev.preventDefault();
  if (ev.shiftKey) nbRedo(); else nbUndo();
});

/* ── 저장(늦춰서) ──────────────────────────────────────────── */
function ntMemoTouch() {
  const m = ntMemoCur();
  if (!m) return;
  ...
  ntMemoTimer = setTimeout(ntMemoSave, 600);
  ntMemoCount();
}
function ntMemoSave() {
  clearTimeout(ntMemoTimer);
  ntMemoTimer = null;
  ntMemoStore();
  if ($('ntMemoSaved').textContent !== t('저장 공간이 꽉 찼어요', 'Storage is full')) {
    $('ntMemoSaved').textContent = t('저장됨', 'Saved');
  }
  ntMemoDrawList();
  nbHistCheckpoint(ntMemoCur());
}
/* 화면을 떠나거나 탭을 닫을 때는 기다리지 않는다. */
addEventListener('visibilitychange', () => { if (document.hidden && ntMemoTimer) ntMemoSave(); });
...
$('ntCatPopName').addEventListener('input', () => { $('ntCatPopErr').hidden = true; });
...
$('ntCatPopSave').addEventListener('click', () => {
  ...
  const err = $('ntCatPopErr');
  if (!name) {
    err.textContent = t('이름을 적어 주세요', 'Enter a name');
    err.hidden = false;
    return $('ntCatPopName').focus();
  }
  const dup = ntCatFind(name);
  if (dup && dup !== ntCatPopCur) {
    /* 예전엔 여기서 그냥 조용히 입력칸에 포커스만 돌아갔다 — 만들기를
       눌러도 아무 반응이 없어 보여서 "단추가 고장났다"는 제보로
       이어졌다. 왜 안 만들어지는지 말해 준다. */
    err.textContent = t('이미 있는 갈래예요', 'That category already exists');
    err.hidden = false;
    $('ntCatPopName').focus();
    return;
  }
...
/* ── 목록 ──────────────────────────────────────────────────── */
function ntMemoDrawList() {
  const rows = ntMemoVisible();
  const box = $('ntMemoList');
  box.textContent = '';
  rows.forEach((m) => {
    const b = document.createElement('button');
    b.type = 'button';
...
function ntMemoDrawDoc() {
  const m = ntMemoCur();
  $('ntMemo').classList.toggle('no-doc', !m);
  $('ntMemoNone').textContent = t('왼쪽에서 메모를 고르거나 「새 메모」를 눌러 시작하세요.',
                                  'Pick a note on the left, or press "New note".');
  /* 큰 검정 띠 단추가 목록 맨 위를 다 먹고 있었다. + 하나로 줄인다 —
     무엇을 하는 단추인지는 자리(목록 머리)가 이미 말한다. */
  $('ntMemoNew').textContent = '+';
  $('ntMemoNew').title = t('새 노트', 'New note');
  $('ntMemoNew').setAttribute('aria-label', t('새 노트', 'New note'));
...
function ntMemoDraw() {
  $('ntIntro').textContent = t(
    '공부하다 떠오른 것을 적어 두는 나만의 자리예요. 갈래로 나눠 정리할 수 있고, 로그인 없이 이 브라우저에만 남습니다.',
    'Your own place for whatever comes up while you study. Sort it into folders — it stays in this browser, no sign-in.');
  ntMemoDrawCats();
  ntMemoDrawList();
  ntMemoDrawDoc();
}
...
/* 들여쓰기(Tab) 최대 단계. 한없이 들어가면 좁은 화면에서 글이 밀려
   안 보이게 된다 — 노션도 딱 이만큼서 더는 안 들어간다. */
const NB_IND_MAX = 4;
const NB_IND_PX = 28;
const NB_TYPES = [
...
const nbNew = (t = 'text', s = '') => ({ id: nbId(), t, s, done: false, c: '', bg: '', cap: '', ind: 0 });
...
function nbToText(blocks) {
  return (blocks || []).map((b) => {
    ...
    if (b.t === 'h2') return `${pad}## ${s}`;
    if (b.t === 'h3') return `${pad}### ${s}`;
    if (b.t === 'ul') return `${pad}- ${s}`;
    if (b.t === 'ol') return `${pad}1. ${s}`;
    if (b.t === 'todo') return `${pad}- [${b.done ? 'x' : ' '}] ${s}`;
    if (b.t === 'quote') return `${pad}> ${s}`;
    if (b.t === 'call') return `${pad}💡 ${s}`;
    if (b.t === 'warn') return `${pad}⚠️ ${s}`;
    if (b.t === 'tip') return `${pad}📌 ${s}`;
...
function nbRow(m, b, i) {
  ...
  row.dataset.b = b.id;
  if (b.ind) row.style.marginLeft = `${Math.min(b.ind, NB_IND_MAX) * NB_IND_PX}px`;
  ...
  /* 어휘/단어 카드 (vocab). 표제어(s)와 뜻풀이(a)를 예쁜 카드로 묶는다. */
  if (b.t === 'vocab') {
    const wrap = document.createElement('div');
    wrap.className = 'nb-vocab-card';

    const top = document.createElement('div');
    top.className = 'nb-vocab-top';
    const tag = document.createElement('span');
    tag.className = 'nb-vocab-tag';
    tag.textContent = '🏷️ ' + t('단어', 'Word');
    const word = document.createElement('div');
    word.className = 'nb-vocab-word nb-txt';
    word.contentEditable = 'true';
    word.spellcheck = false;
    word.dataset.ph = t('단어 / 발음 (예: 기억나다)', 'Word / Pronunciation');
    word.dataset.f = 's';
    word.innerHTML = nbClean(b.s || '');
    nbWireQA(word, m, b, 's', 'a');
    top.append(tag, word);

    const bot = document.createElement('div');
    bot.className = 'nb-vocab-bot';
    const def = document.createElement('div');
    def.className = 'nb-vocab-def nb-txt';
    def.contentEditable = 'true';
    def.spellcheck = false;
  ...
  /* 예문 (ex). 한국어 문장(s)과 번역/해석(a)을 상하로 깔끔하게 정렬한다. */
  if (b.t === 'ex') {
    const wrap = document.createElement('div');
    wrap.className = 'nb-ex-box';

    const koLine = document.createElement('div');
    koLine.className = 'nb-ex-ko-line';
    const koMark = document.createElement('span');
    koMark.className = 'nb-ex-mark';
    koMark.textContent = '🇰🇷';
    const koTxt = document.createElement('div');
    koTxt.className = 'nb-ex-ko nb-txt';
    koTxt.contentEditable = 'true';
    koTxt.spellcheck = false;
    koTxt.dataset.ph = t('한국어 예문 (예: 비가 와서 우산을 썼어요.)', 'Korean sentence');
    koTxt.dataset.f = 's';
    koTxt.innerHTML = nbClean(b.s || '');
    nbWireQA(koTxt, m, b, 's', 'a');
    koLine.append(koMark, koTxt);

    const trLine = document.createElement('div');
    trLine.className = 'nb-ex-tr-line';
    const trMark = document.createElement('span');
    trMark.className = 'nb-ex-mark-sub';
    trMark.textContent = '↳';
    const trTxt = document.createElement('div');
    trTxt.className = 'nb-ex-tr nb-txt';
    trTxt.contentEditable = 'true';
    trTxt.spellcheck = false;
  ...
  /* 접이식 토글 (toggle). 질문/제목(s)과 숨길 내용/정답(a) */
  if (b.t === 'toggle') {
    const wrap = document.createElement('div');
    wrap.className = 'nb-toggle-box' + (b.open ? ' open' : '');

    const headLine = document.createElement('div');
    headLine.className = 'nb-toggle-h';
    const tglBtn = document.createElement('button');
    tglBtn.type = 'button';
    tglBtn.className = 'nb-toggle-arrow';
    tglBtn.textContent = b.open ? '▼' : '▶';
    tglBtn.setAttribute('aria-label', t('토글 열기/닫기', 'Toggle open/close'));
    ...
    const sumTxt = document.createElement('div');
    sumTxt.className = 'nb-toggle-sum nb-txt';
    sumTxt.contentEditable = 'true';
    sumTxt.spellcheck = false;
    sumTxt.dataset.ph = t('토글 제목 / 질문 (항상 보임)', 'Toggle title / Question');
    sumTxt.dataset.f = 's';
    sumTxt.innerHTML = nbClean(b.s || '');
    nbWireQA(sumTxt, m, b, 's', 'a');
    headLine.append(tglBtn, sumTxt);

    const bodyWrap = document.createElement('div');
    bodyWrap.className = 'nb-toggle-body';
    if (!b.open) bodyWrap.hidden = true;

    const dtlTxt = document.createElement('div');
    dtlTxt.className = 'nb-toggle-dtl nb-txt';
    dtlTxt.contentEditable = 'true';
    dtlTxt.spellcheck = false;
  ...
  if (b.t === 'ul' || b.t === 'ol' || b.t === 'call' || b.t === 'howto' || b.t === 'warn' || b.t === 'tip') {
    const mark = document.createElement('div');
    mark.className = 'nb-mark';
    mark.textContent = b.t === 'ul' ? '•' : b.t === 'ol' ? `${nbOrd(m, i)}.` : b.t === 'howto' ? '📐' : b.t === 'warn' ? '⚠️' : b.t === 'tip' ? '📌' : '💡';
...
const NB_SAMPLE = (t0) => ({
  ...
  warn: t('주의: 「안 되다」와 「안되다」는 띄어쓰기에 따라 뜻이 달라져요.', 'Watch out: 「안 되다」 and 「안되다」 have different meanings based on spacing.'),
  tip: t('팁: 비슷한 표현으로 「-기 때문에」를 함께 알아두면 좋아요.', 'Tip: It is great to learn the similar expression 「-기 때문에」 together.'),
  howto: t('A/V + 아서/어서', 'A/V + 아서/어서'),
  code: t('먹다 → 먹어요', '먹다 → 먹어요'),
}[t0] ?? null);
const NB_SAMPLE_TWO = (t0) => ({
  qa: {
    s: t('이 표현은 언제 써요?', 'When do you use this expression?'),
    a: t('친한 사이에서 편하게 말할 때 써요.', 'You use it casually between close friends.'),
  },
  vocab: {
    s: t('기억나다 [기엉나다]', '기억나다 [to remember]'),
    a: t('동사. 잊었던 생각이나 사실이 머릿속에 다시 떠오르다.', 'Verb. To recall or remember something previously forgotten.'),
  },
  ex: {
    s: t('한국 친구 덕분에 한국어 실력이 많이 늘었어요.', '한국 친구 덕분에 한국어 실력이 많이 늘었어요.'),
    a: t('Thanks to my Korean friend, my Korean skills have improved a lot.', 'Thanks to my Korean friend, my Korean skills have improved a lot.'),
  },
  toggle: {
...
/* 사진 칸(b)에 file 을 앉힌다. 성공하면 참을 준다 — 실패하면 b 를
   건드리지 않는다(호출한 쪽이 새로 만든 칸을 그때 버릴 수 있게). */
async function nbImgSetFromFile(b, file) {
  if (!file.type.startsWith('image/')) {
...
async function nbImgPick(m, b, input) {
  ...
  input.value = '';
  if (!file) return;
  if (await nbImgSetFromFile(b, file)) nbSave(m);
}
/* 글 블록에 사진을 그대로 붙여넣을 때(Cmd/Ctrl+V) 쓴다 — 사진을 넣으려면
   매번 「/」로 종류를 사진으로 바꾸고 파일 고르개까지 눌러야 하는 게
   번거롭다는 제보로 만들었다. 빈 글 블록이면 그 자리에서 사진 칸으로
   바뀌고, 이미 글이 있으면 지우지 않고 바로 아래에 새 사진 칸을 만든다. */
async function nbImgFromClipboard(m, b, el, file) {
  const blocks = nbBlocks(m);
  const i = blocks.indexOf(b);
  const reuse = b.t === 'text' && !nbPlain(el.innerHTML);
  const target = reuse ? b : nbNew('img');
  const ok = await nbImgSetFromFile(target, file);
  if (!ok) return;
  const url = target.s;
  if (reuse) {
    /* nbSave() 가 부를 nbRender() 는 지금 포커스가 가 있는 el 을 통째로
       지운다 — 그러면 el 의 blur 손잡이가 뒤늦게 걸려 b.s 를 (아직
       비어 있던) el.innerHTML 로 되돌려 버린다. 그 blur 를 먼저
       흘려보낸 다음에 사진 주소를 앉힌다 — nbMdCheck 에서 겪은 것과
       같은 버그다. */
    el.blur();
    b.t = 'img'; b.s = url; b.cap = '';
  } else {
...
function nbOrd(m, i) {
  const blocks = nbBlocks(m);
  const ind = blocks[i].ind || 0;
  let n = 1;
  /* 같은 들여쓰기 단계끼리만 센다 — 한 단계 더 들어간(중첩된) 목록은
     건너뛰고, 한 단계 덜 들어간 데까지 올라가면 거기서 멈춘다(그
     위는 이 목록이 아니다). */
  for (let k = i - 1; k >= 0; k--) {
    const bkInd = blocks[k].ind || 0;
    if (bkInd < ind) break;
    if (bkInd > ind) continue;
    if (blocks[k].t !== 'ol') break;
    n++;
  }
  return n;
}
...
function nbWire(el, m, b) {
  ...
  el.addEventListener('keydown', (ev) => {
    ...
    if (ev.key === 'Enter' && !ev.shiftKey) {
      ...
      next.c = b.c; next.bg = b.bg; next.ind = b.ind || 0;
      blocks.splice(i + 1, 0, next);
      nbFocus = next.id;
      return nbSave(m2);
    }
    if (ev.key === 'Backspace' && nbCaretAt0(el)) {
      /* 들여쓴 줄이면 지우기·종류 되돌리기보다 내어쓰기가 먼저다 — 노션과
         같다. 한 번에 확 밀리면(내어쓰기 없이 바로 글로 바뀌거나 지워지면)
         한 단계씩 무르기가 안 된다. */
      if (b.ind) {
        ev.preventDefault();
        b.ind -= 1;
      ...
      if (['ul', 'ol', 'todo', 'quote', 'call', 'warn', 'tip', 'howto', 'code', 'h1', 'h2', 'h3'].includes(b.t)) {
        ev.preventDefault();
        b.t = 'text'; b.done = false;
        nbFocus = b.id; nbAtEnd = false;
        return nbSave(m2);
    ...
    /* 들여쓰기(Tab)·내어쓰기(Shift+Tab). 바로 위 블록보다 한 단계 넘게는
       못 들어간다 — 안 그러면 사이가 붕 뜬 들여쓰기가 생긴다. */
    if (ev.key === 'Tab') {
      ev.preventDefault();
      if (ev.shiftKey) {
        if (b.ind) { b.ind -= 1; nbFocus = b.id; nbAtEnd = true; nbSave(m2); }
      } else {
        const prevInd = i > 0 ? (blocks[i - 1].ind || 0) : -1;
        const maxInd = Math.min(prevInd + 1, NB_IND_MAX);
        if ((b.ind || 0) < maxInd) { b.ind = (b.ind || 0) + 1; nbFocus = b.id; nbAtEnd = true; nbSave(m2); }
      }
      return;
    }
    ...
    /* Cmd/Ctrl+K 로 링크를 건다 — 고른 글자가 있어야 하고, 없다면
       커서가 이미 걸린 링크 위일 때만(그 링크를 고치거나 없애려고) 연다. */
    if ((ev.metaKey || ev.ctrlKey) && ev.key.toLowerCase() === 'k') {
      ev.preventDefault();
      const range = nbLinkTargetRange(el);
      if (range) nbLinkOpen(el, range, b);
    }
  ...
  /* 붙여넣기는 글자만 받는다 — 남의 쪽에서 통째로 끌고 오는 서식이
     노트를 제 마음대로 칠하지 않게. 다만 사진을 복사해 붙여넣은 경우는
     예외 — 그건 글자가 없으니 그냥 두면 아무 일도 안 일어난 것처럼
     보인다("사진을 넣을 수가 없어" 제보의 원인). */
  el.addEventListener('paste', (ev) => {
    const cd = ev.clipboardData || window.clipboardData;
    const imgItem = cd && [...cd.items || []].find((it) => it.kind === 'file' && it.type.startsWith('image/'));
    if (imgItem) {
      ev.preventDefault();
      const file = imgItem.getAsFile();
      const m2 = ntMemoCur();
      if (file && m2) nbImgFromClipboard(m2, b, el, file);
      return;
    }
    const txt = cd.getData('text/plain');
    /* 순수 주소만 붙여넣으면 자동으로 링크가 된다(문장 속에 섞여 있으면
       그냥 글자로 받는다 — 마음대로 서식을 입히면 오히려 성가시다).
       고른 글자가 있으면 구글 문서·노션처럼 그 위에 링크를 걸고, 없으면
       주소 자체를 누를 수 있는 글자로 넣는다. */
    if (/^https?:\/\/\S+$/i.test(txt.trim())) {
      const url = nbLinkNormalize(txt.trim());
      if (url) {
        ev.preventDefault();
        ...
        if (sel && sel.rangeCount && !sel.getRangeAt(0).collapsed) {
          ...
          document.execCommand('insertHTML', false,
            `<a href="${url.replace(/"/g, '&quot;')}">${txt.trim().replace(/&/g, '&amp;').replace(/</g, '&lt;')}</a>`);
        }
        const node = getSelection().anchorNode;
        const host = node && (node.nodeType === 3 ? node.parentElement : node);
        const a = host && host.closest && host.closest('a[href]');
...
function nbMenuOpen(anchor, m, b, only) {
  ...
  if (only !== 'type') {
    ...
    sep();
    /* Tab·Shift+Tab 은 키보드가 있어야 편하다 — 폰 화면 키보드는 Tab이
       없거나 포커스만 옮기는 경우가 많아서, 손잡이 메뉴에도 같은 일을
       하는 단추를 둔다. 지금 물릴 수 없는 쪽(맨 위 단계에서 내어쓰기,
       위 블록보다 더 들어갈 수 없을 때 들여쓰기)은 아예 안 보여준다. */
    const blocksN = nbBlocks(m);
    const iN = blocksN.indexOf(b);
    const prevIndN = iN > 0 ? (blocksN[iN - 1].ind || 0) : -1;
    const maxIndN = Math.min(prevIndN + 1, NB_IND_MAX);
    if (b.ind) item('⇤', t('내어쓰기', 'Outdent'), () => { b.ind -= 1; nbMenuClose(); nbSave(m); });
    if ((b.ind || 0) < maxIndN) item('⇥', t('들여쓰기', 'Indent'), () => { b.ind = (b.ind || 0) + 1; nbMenuClose(); nbSave(m); });
    if (b.ind || (b.ind || 0) < maxIndN) sep();
...
/* 페이지가 스크롤돼 손잡이(anchor)가 움직이면 뜬 메뉴가 엉뚱한 자리에
   남으니 닫는다. 그런데 이 메뉴 자신도 목록이 길면 overflow-y:auto 로
   제 안에서 스크롤한다 — capture:true 로 window 에 걸어 두면 그 스크롤도
   똑같이 걸려서, 메뉴를 내리려고 하는 순간 메뉴가 저 스스로를 닫아
   버렸다(실제로 겪은 버그 — 목록 아래쪽 항목에 닿지도 못했다). 스크롤이
   일어난 자리가 메뉴 자신이면 넘어간다. */
window.addEventListener('scroll', (ev) => { if (nbMenuAt && ev.target !== $('nbMenu')) nbMenuClose(); }, true);
...
/* ── 링크 걸기(Cmd/Ctrl+K) ───────────────────────────────────
   주소가 http(s) 가 아니면(javascript: 따위) 막는다 — nbClean 이 저장
   시점에 한 번 더 걸러내지만, 여기서도 미리 막아 둔다(방어를 겹친다). */
function nbLinkNormalize(raw) {
...
/* 지금 고른 자리(또는 커서가 놓인 링크)를 Range 로 돌려준다. 아무것도
   고르지 않았고 링크 위도 아니면 null — 걸 자리가 없다. */
function nbLinkTargetRange(el) {
  const sel = getSelection();
  ...
  const a = host && host.closest && host.closest('a');
  if (!a || !el.contains(a)) return null;
  const full = document.createRange();
  full.selectNodeContents(a);
  return full;
}
let nbLinkRange = null;
let nbLinkTarget = null;   // { el, b }
function nbLinkClose() {
  $('nbLinkPop').hidden = true;
  nbLinkRange = null;
  nbLinkTarget = null;
}
function nbLinkOpen(el, range, b) {
  nbLinkRange = range.cloneRange();
  nbLinkTarget = { el, b };
  const host = range.startContainer.nodeType === 3 ? range.startContainer.parentElement : range.startContainer;
  const a = host && host.closest && host.closest('a');
  const cur = (a && el.contains(a) && a.textContent === range.toString()) ? (a.getAttribute('href') || '') : '';
  const inp = $('nbLinkUrl');
  inp.value = cur;
  inp.placeholder = 'https://…';
  $('nbLinkRemove').hidden = !cur;
  $('nbLinkRemove').textContent = t('링크 없애기', 'Remove link');
  $('nbLinkApply').textContent = t('적용', 'Apply');
  const box = $('nbLinkPop');
  box.hidden = false;
...
/* execCommand('createLink'/'unlink') 를 쓰려면 브라우저 선택 영역이
   다시 그 Range 를 가리키고 있어야 한다 — 링크 판의 입력칸에 포커스가
   가 있는 동안 선택은 이미 풀려 있으므로 여기서 되돌린다. */
function nbLinkRestoreSel() {
  const { el } = nbLinkTarget;
  el.focus();
  const sel = getSelection();
  sel.removeAllRanges();
  sel.addRange(nbLinkRange);
}
function nbLinkApply() {
  if (!nbLinkRange || !nbLinkTarget) return;
  const url = nbLinkNormalize($('nbLinkUrl').value);
  if (!url) { $('nbLinkUrl').focus(); return; }
  const { el, b } = nbLinkTarget;
  nbLinkRestoreSel();
  document.execCommand('createLink', false, url);
  /* 방금 만든 <a> 를 찾아 새 창으로 열리게 다듬는다 — contenteditable
     안에서는 그냥 눌러서는 안 따라가고 Cmd/Ctrl+클릭이라야 여는데, 그건
     브라우저 기본이라 target·rel 만 붙여 두면 된다. */
  const sel = getSelection();
...
$('nbLinkApply').addEventListener('click', nbLinkApply);
$('nbLinkRemove').addEventListener('click', nbLinkRemoveFn);
$('nbLinkUrl').addEventListener('keydown', (ev) => {
  if (ev.key === 'Enter') { ev.preventDefault(); nbLinkApply(); }
  if (ev.key === 'Escape') { ev.preventDefault(); nbLinkClose(); }
});
document.addEventListener('click', (ev) => {
  if (!nbLinkRange) return;
  if (ev.target.closest('#nbLinkPop')) return;
  nbLinkClose();
});

/* ── 문서 단추들 ───────────────────────────────────────────── */
$('ntMemoNew').addEventListener('click', ntMemoNew);
$('ntMemoBack').addEventListener('click', () => {
  if (ntMemoTimer) ntMemoSave();
...
/* ── 노트에서 단어장으로 ─────────────────────────────────────
   적어 두다 「이건 외워야겠다」 싶은 말이 나온다. 그때 단어장을 따로
   열어 다시 치게 하지 않는다.

   무엇을 집는가 — 글자를 골라 뒀으면 그것, 아니면 커서가 놓인 줄,
   그것도 비었으면 제목. 목록표(- )나 제목표(# )는 떼고 본다.

   「집 - house」처럼 적어 둔 줄은 가운뎃점에서 갈라 뜻까지 채운다.
   뜻을 안 적었으면 사전에서 찾아 채우되, 사전에 없으면 **빈 칸으로
...
function ntMemoSplit(text) {
  const clean = String(text).replace(/^\s*(#{1,3}\s+|[-*]\s+(\[[ xX]\]\s*)?|>\s?)/, '').trim();
  /* 가르는 표 뒤에는 반드시 공백이 와야 한다 — 그래야 「-고 싶다」처럼
     붙임표로 시작하는 문법 표현이 통째로 낱말로 남는다. */
  const m = clean.match(/^(.+?)\s*(?:—|–|-|:|=)\s+(.+)$/);
  return m ? { word: m[1].trim(), mean: m[2].trim() } : { word: clean, mean: '' };
}
$('ntMemoWb').addEventListener('click', async () => {
  const picked = ntMemoPickText();
  ...
  open('wordbook');
  openWordForm({
    word: (g?.head || word).slice(0, 200),
    meaning: (mean || g?.meaning || '').slice(0, 300),
    tag: g?.tag || '',
  });
});

$('ntMemoPin').addEventListener('click', () => {
  const m = ntMemoCur();
  if (!m) return;
  m.pin = !m.pin;
  ntMemoStore();
...
function ntPrintBody(blocks) {
  ...
  (blocks || []).forEach((b) => {
    ...
    if (b.t === 'ex') {
      flush();
      const ko = nbPlain(b.s || ''), tr = nbPlain(b.a || '');
      if (!ko && !tr) return;
      out.push(`<div class="ntp-ex"><div class="ntp-ex-ko">🇰🇷 ${nbClean(b.s)}</div>${tr ? `<div class="ntp-ex-tr">↳ ${nbClean(b.a)}</div>` : ''}</div>`);
      return;
    }
    if (b.t === 'toggle') {
      flush();
      const sum = nbPlain(b.s || ''), dtl = nbPlain(b.a || '');
      if (!sum && !dtl) return;
      out.push(`<details class="ntp-toggle" open><summary class="ntp-toggle-s">${nbClean(b.s)}</summary><div class="ntp-toggle-b">${nbClean(b.a || '')}</div></details>`);
      return;
    }
    ...
    if (b.t === 'warn') {
      out.push(`<div class="ntp-warn"><span class="ntp-warn-i">⚠️</span><span${cls}>${html}</span></div>`);
      return;
    }
    if (b.t === 'tip') {
      out.push(`<div class="ntp-tip"><span class="ntp-tip-i">📌</span><span${cls}>${html}</span></div>`);
      return;
    }
...
const NT_HELP_ROWS = [
  ...
  { key: '[] ',  ko: '할 일 체크박스로 바꾸기',             en: 'Turn into a to-do item' },
  { key: '> ',   ko: '인용으로 바꾸기',                    en: 'Turn into a quote' },
  { key: '! ',   ko: '주의 블록으로 바꾸기',               en: 'Turn into a warning block' },
  { key: '? ',   ko: '팁 블록으로 바꾸기',                 en: 'Turn into a tip block' },
  { key: '--- ', ko: '구분선으로 바꾸기',                  en: 'Turn into a divider' },
  { key: 'Tab',  ko: '들여쓰기 (최대 4단계)',              en: 'Indent (up to 4 levels)' },
  { key: 'Shift+Tab', ko: '내어쓰기',                       en: 'Outdent' },
...
/* ── 전체화면 집중 모드 ────────────────────────────────────────
   .nt-memo-doc 에 자리만 바꾸는 클래스를 얹는다 — 문서를 옮기거나
   다시 그리지 않으므로 열고 닫아도 쓰던 것을 잃지 않는다. 헤더·히어로·
   노트 목록이 다 가려지고 제목·글만 남는다(집중). */
let ntFullOn = false;
/* 펼침·접힘 두 그림. 안쪽으로 모이는 꺾쇠(접힘)와 바깥으로 벌어지는
   꺾쇠(펼침) — 다른 아이콘 자리와 같은 선 굵기(1.5)로 맞춘다. */
const NT_ICON_MAXIMIZE = '<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M7 3H4a1 1 0 0 0-1 1v3"/><path d="M13 3h3a1 1 0 0 1 1 1v3"/><path d="M7 17H4a1 1 0 0 1-1-1v-3"/><path d="M13 17h3a1 1 0 0 0 1-1v-3"/></svg>';
const NT_ICON_MINIMIZE = '<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 7V4a1 1 0 0 1 1-1h3"/><path d="M16 7V4a1 1 0 0 0-1-1h-3"/><path d="M4 13v3a1 1 0 0 0 1 1h3"/><path d="M16 13v3a1 1 0 0 1-1 1h-3"/></svg>';
function ntFullSet(on) {
  ntFullOn = on;
  $('ntMemoDoc').classList.toggle('nt-full', on);
  document.body.classList.toggle('nt-full-open', on);
  $('ntMemoFull').classList.toggle('on', on);
  $('ntMemoFull').innerHTML = on ? NT_ICON_MINIMIZE : NT_ICON_MAXIMIZE;
  $('ntMemoFull').title = on ? t('전체화면 끄기', 'Exit fullscreen') : t('전체화면으로 크게 쓰기', 'Write in fullscreen');
  if (on) $('ntMemoTitle').focus();
}
$('ntMemoFull').addEventListener('click', () => ntFullSet(!ntFullOn));
document.addEventListener('keydown', (ev) => { if (ev.key === 'Escape' && ntFullOn) ntFullSet(false); });
/* 목록으로 나가거나 다른 노트를 고르면 전체화면도 접는다 — 켠 채로
   다른 노트를 열면 「내가 지금 어디 있는지」가 헷갈린다. */
$('ntMemoBack').addEventListener('click', () => ntFullSet(false));
...
$('ntMemoDup').addEventListener('click', () => {
  ...
  if (ntMemoTimer) ntMemoSave();
  const now = Date.now();
  const copy = { ...m, id: ntMemoNewId(), pin: false, at: now, up: now,
                 title: t(`${ntMemoName(m)} (사본)`, `${ntMemoName(m)} (copy)`) };
  ntMemo.unshift(copy);
  ntMemoStore();
  ntMemoOpen(copy.id);
});
...
/* 예문 만들기에서 노트로 건너간다. 사이드 메뉴(learn/notes)에도 같은
   길이 있어서 어느 화면에서든 열 수 있다. */
$('sbToNotes').addEventListener('click', () => openSection('notes'));

async function drawSentenceHead() {
  await sbNeed();
  $('sbToNotes').textContent = '🗒️ ' + t('내 노트 열기', 'Open my notes');
...
async function drawSections() {
  ...
  const card = (s) => {
    ...
    if (s.id === 'courses') {
      ...
      foot =
        ...
        '</div>';
    } else if (!s.ready && !s.go) {
      foot = `<div class="lc-order">${t('아직 준비 중이에요.', 'Not ready yet.')}</div>`;
    }
    return (
      `<button class="lc-card" ${s.go ? `data-learn-go="${esc(s.go)}"` : `data-section="${s.id}"`}>` +
        '<div class="lc-top">' +
  ...
  /* 묶음에 빠진 갈래가 있으면 맨 끝에 「그 밖」으로 세운다 — 갈래를 새로
  ...
  let mine = '';
  try { mine = myCourseHtml(); } catch (e) { mine = ''; }   // 내 코스가 깨져도 갈래 목록은 선다
  $('learnView').classList.toggle('has-level', !!ltLoadLevel());
  $('lsecList').innerHTML = mine + groups.map((g) =>
    `<h3 class="lsec-group">${esc(secTx(g.title))}</h3>` +
    g.items.map((x) => typeof x === 'string' ? LEARN_SECTIONS.find((s) => s.id === x) : x)
...
function rdCard(r, score) {
  ...
  /* 지문을 소리로도 들을 수 있게.
     **녹음이 지문 id 로 assets/audio/read/ 에 이미 다 있다**(54편 전부).
     say() 는 경로를 안 주면 낱말 텍스트로 파일 이름을 지어 찾는데, 지문은
     그 규칙과 이름이 다르다 — 그래서 처음엔 녹음이 있는데도 못 찾고 로봇
     목소리로 떨어지고 있었다. 여기서 경로를 못 박아 준다. 파일이 없는
     지문이 생겨도 say() 가 조용히 Web Speech 로 넘어간다. */
  const listen = document.createElement('button');
  listen.type = 'button';
  listen.className = 'rd-say';
  listen.dataset.say = r.passage;
  listen.dataset.audio = `${AUDIO_BASE}read/${r.id}.mp3`;
...
function cvTurnBlock(c) {
  ...
  listen.dataset.audio = `${AUDIO_BASE}convo/${turn.id}.mp3`;
...
/* ── 여행 한국어 (Travel Korean) ───────────────────────────────
   공항·교통·숙소·식당·쇼핑·관광·긴급 7대 상황별
   쉬운 말 / 보통 말 / 정확한 말 3단계 실전 구문과 필수 단어장. */
let TRAVEL_CATEGORIES = null;
let TRAVEL_PHRASES = null;
let TRAVEL_VOCAB = null;
let tvP = null;
const tvNeed = () => (tvP ??= import('./travel-data.js?v=d0eca8e9').then((m) => {
...
async function drawTravel() {
  ...
  if (lvlBox) {
    lvlBox.textContent = '';
    const lvls = [
      { id: 'easy', emoji: '🟢', ko: '쉬운 버전 (단어)', en: 'Easy (Keywords)' },
      { id: 'std',  emoji: '🟡', ko: '보통 버전 (존댓말)', en: 'Standard (Polite)' },
      { id: 'pro',  emoji: '🟣', ko: '정확한 버전 (격식)', en: 'Accurate (Formal)' },
      { id: 'all',  emoji: '📋', ko: '3단계 전체', en: 'All 3 Tiers' },
    ];
    const segWrap = document.createElement('div');
    segWrap.className = 'tv-seg-control';
    segWrap.setAttribute('role', 'tablist');
    segWrap.setAttribute('aria-label', t('난이도 선택', 'Difficulty selection'));

    lvls.forEach((l) => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = `tv-seg-btn is-${l.id}${tvLvl === l.id ? ' active' : ''}`;
      btn.setAttribute('role', 'tab');
      btn.setAttribute('aria-selected', tvLvl === l.id ? 'true' : 'false');
      btn.innerHTML = `<span class="tv-seg-ico">${l.emoji}</span><span class="tv-seg-txt">${esc(t(l.ko, l.en))}</span>`;
      btn.addEventListener('click', () => {
        tvLvl = l.id;
        drawTravel();
      });
  ...
  if (listBox) {
    ...
    list.forEach((p) => {
      ...
      if (tvLvl === 'all') {
        ...
        [
          ...
          r.innerHTML = `<span class="tv-card-badge ${row.key}" style="flex:none; margin-top:2px;">${row.ko}</span>` +
                        ...
                        `<button type="button" class="tv-mini-audio" style="border:none; background:none; cursor:pointer; font-size:15px; padding:2px 6px; color:var(--orange-deep);" title="${esc(t('듣기', 'Listen'))}">🔊</button>`;
          const miniPlay = r.querySelector('button');
          miniPlay.addEventListener('click', (ev) => {
            ev.stopPropagation();
            say(row.item.ko, `${AUDIO_BASE}travel/${row.key}/${p.id}.webm`);
          });
          stack.appendChild(r);
        });
      ...
      audioBtn.addEventListener('click', () => {
        const curTier = tvLvl === 'all' ? 'std' : tvLvl;
        say(curItem.ko, `${AUDIO_BASE}travel/${curTier}/${p.id}.webm`);
      });
    ...
    if (words.length) {
      ...
      words.forEach((w) => {
        ...
        chip.title = t('발음 듣기', 'Listen');
        chip.innerHTML = `<b>${esc(w.ko)}</b><span>(${esc(w.en)})</span> 🔊`;
        chip.addEventListener('click', () => say(w.ko));
...
const GUIDES = {
  travel: {
    emoji: '✈️',
    steps: [
      { ko: ['상황 고르기', '공항·교통·식당·쇼핑 등 지금 마주친 여행 상황을 탭하세요.'],
        en: ['Pick a situation', 'Tap the situation you are facing right now — airport, transit, dining, etc.'] },
      { ko: ['난이도 버전 선택', '생초보는 쉬운 말(단어), 기본은 보통 말, 격식은 정확한 말을 골라 연습하세요.'],
        en: ['Choose your level', 'Pick Easy for keywords, Standard for polite conversation, or Accurate for formal Korean.'] },
      { ko: ['소리 듣고 바로 말하기', '소리를 듣고 따라 말해보세요. 팜플렛을 다운로드해 여행 중에 들고 다닐 수도 있어요.'],
        en: ['Listen and speak', 'Listen to native pronunciation. You can also download the PDF brochure to take with you.'] },
    ],
  },
  courses: {
    emoji: '📚',
  ...
  notes: {
    emoji: '🗒️',
    steps: [
      { ko: ['적고 싶을 때 바로', '공부하다 떠오른 문장·문법을 여기에 적어 둡니다. 사이드 메뉴에서 어느 화면에서든 열 수 있어요.'],
        en: ['Whenever something comes up', 'Write down sentences and grammar as you study. Open it from the side menu on any screen.'] },
      { ko: ['갈래로 나눠 정리하기', '「문법」·「어휘」처럼 갈래를 만들어 노트를 묶어 둘 수 있어요.'],
        en: ['Sort into folders', 'Make folders like "Grammar" or "Vocabulary" and group your notes.'] },
      { ko: ['나만 봅니다', '로그인 없이 이 브라우저에만 남아요. 기기를 옮길 땐 내려받기로 챙기세요.'],
        en: ['Only you see it', 'It stays in this browser without signing in. Use Download to take it with you.'] },
    ],
  },
  convo: {
    emoji: '💬',
...
async function openSection(id, quiet) {
  ...
  $('topikHdBtn').classList.toggle('on', s.id === 'topik');
  $('myHdBtn').classList.toggle('on', s.id === 'mine');
  $('learnBtn').classList.toggle('on', s.id !== 'topik' && s.id !== 'mine');
...
async function openLearnSub(sub) {
  ...
  /* 옛 주소 #learn/writing 을 새 자리로 넘긴다. 쓰기가 별도 갈래였을 때
     ...
     문항 번호(#learn/writing/51-1)가 붙어 있으면 그 자리를 그대로 들고
     간다 — topik-writing/ 정적 쪽에서 "직접 써 보기" 를 눌렀을 때
     방금 읽던 문항이 아니라 목록으로 떨어지면, 그 사이 손이 식는다. */
  ...
  if (secId === 'topik' && rest.length) {
    ...
    if (wantExam || wantSkill) {
      ...
      openSection('topik', true);
      /* 문항 하나까지 지정됐으면(#learn/writing/51-1 · #learn/topik/I/
         reading/tr-001 · #learn/topik/II/listening/lII-04) 목록이 아니라
         그 문항을 바로 연다 — topik-writing/·topik-reading/·
         topik-listening/ 정적 쪽의 "직접 풀어보기"가 방금 읽던 문항
         그대로 이어지게 하려는 것이다. tqShowSkill 이 자료를 받아 오길
         기다린 뒤에야 찾을 수 있다. */
      const wantItem = wantSkill ? rest[wantExam ? 2 : 1] : null;
      await tqShowSkill(true);
      if (wantItem && wantSkill === 'writing') {
        const it = TW_ITEMS.find((x) => x.id === wantItem);
        // 못 찾아도 조용히 목록에 남는다 — 지어낸 문항을 열 수는 없다.
        if (it) { twQ = it.q; twDraw(); twOpen(it); }
      } else if (wantItem && wantSkill === 'reading') {
        const it = tqE().reading.find((x) => x.id === wantItem);
        if (it) tqOpenOne(it);
      } else if (wantItem && wantSkill === 'listening') {
        const it = tlE().items.find((x) => x.id === wantItem);
        if (it) tlOpenOne(it);
      }
      return;
    }
...
function backToSections() {
  lsecOpen = null;
  $('learnView').classList.remove('in-sec', 'topik-mode');
  window.cpMark('learn');
...
$('lsecList').addEventListener('click', (ev) => {
  ...
  const go = ev.target.closest('[data-learn-go]');
  if (go) { openLearnSub(go.dataset.learnGo); return; }
...
$('lsecWrap').addEventListener('change', (ev) => {
  const q = ev.target.closest('[data-tw-q]');
  if (!q) return;
  twQ = Number(q.dataset.twQ);
  twStopClock(); twItem = null;   // 번호를 바꾸면 열어 둔 문항은 다른 번호 것이라 놓는다
  twDraw();
});
...
async function drawCourses() {
  ...
  /* 숫자 세 칸 대신 진도 한 줄. 코스·레슨·문제 수를 나란히 세우면 볼 것만 늘고
     정작 「내가 어디까지 했나」는 안 보였다. */
  const pctAll = allLessons ? Math.round((doneLessons / allLessons) * 100) : 0;
  $('lcSummary').innerHTML =
    '<div class="cl-sum">' +
      `<div class="cl-sum-t">${esc(t(`코스 ${rows.length}개 · 레슨 ${allLessons}개`, `${rows.length} courses · ${allLessons} lessons`))}</div>` +
      `<div class="cl-bar"><i style="width:${pctAll}%"></i></div>` +
      `<div class="cl-sum-n">${esc(t(`${doneLessons}개 끝냄`, `${doneLessons} done`))}</div>` +
    '</div>';
  if (!rows.length) {
    $('lcList').innerHTML = `<div class="learn-empty">${esc(t('이 단계 코스는 곧 채울게요.', 'Courses for this level are coming soon.'))}</div>`;
    return;
  }
  /* 코스 목록 — 예전엔 코스마다 큰 카드(분류·제목·한 줄 소개·설명·「이 코스 다음」
     안내·진도)였고, 그 위에 단계별 로드맵 카드가 또 있었다. 초급 43개가 세로
     11,000px 를 넘어서 무엇부터 할지가 안 보였다. 로드맵의 단계를 목록의 묶음
     제목으로 쓰고, 코스는 한 줄(그림 · 제목 · 한 줄 소개 · 진도)로 줄인다.
     목록 순서가 곧 권하는 순서이고, 막지는 않는다(앞 코스를 안 해도 열린다). */
  const groups = level === 'beginner'
    ? BEGINNER_ROADMAP.map((p) => ({
        k: t(p.tag.ko, p.tag.en), title: t(p.title.ko, p.title.en),
        list: p.courses.map((id) => rows.find((c) => c.id === id)).filter(Boolean),
      }))
    : [{ k: '', title: '', list: rows }];
  const placed = new Set(groups.flatMap((g) => g.list.map((c) => c.id)));
  const rest = rows.filter((c) => !placed.has(c.id));
  if (rest.length) groups.push({ k: '', title: t('그 밖의 코스', 'More courses'), list: rest });
  // 「이어서」 — 권하는 순서에서 아직 안 끝낸 첫 코스. 하나만 짚는다.
  const nextId = groups.flatMap((g) => g.list).find((c) => courseDone(c) < c.lessons.length)?.id;
  const row = (c) => {
    const n = courseDone(c), all = c.lessons.length, done = n >= all;
    const open = c.id === lcOpenId;
    return `<button type="button" class="cl-row${done ? ' done' : ''}${c.id === nextId ? ' next' : ''}${open ? ' open' : ''}" data-course="${esc(c.id)}" aria-expanded="${open}">` +
      `<span class="cl-e">${courseIcon(c.emoji)}</span>` +
      '<span class="cl-main">' +
        `<span class="cl-t">${esc(cTx(c.title))}</span>` +
        `<span class="cl-s">${esc(cTx(c.tagline))}</span>` +
      '</span>' +
      (c.id === nextId ? `<span class="cl-pill">${esc(t('이어서', 'Up next'))}</span>` : '') +
      `<span class="cl-n">${done ? '✓' : `${n}/${all}`}</span>` +
      '<span class="cl-chev" aria-hidden="true">›</span>' +
    '</button>' +
    (open ? clLessonsHtml(c) : '');
  };
  $('lcList').innerHTML = groups.filter((g) => g.list.length).map((g) => {
    const gd = g.list.reduce((a, c) => a + courseDone(c), 0);
    const ga = g.list.reduce((a, c) => a + c.lessons.length, 0);
    return '<section class="cl-g">' +
      (g.title
        ? '<div class="cl-gh">' +
            `<div>${g.k ? `<div class="cl-gk">${esc(g.k)}</div>` : ''}<div class="cl-gt">${esc(g.title)}</div></div>` +
            `<div class="cl-gn">${gd} / ${ga}</div>` +
          '</div>'
        : '') +
      `<div class="cl-list">${g.list.map(row).join('')}</div>` +
    '</section>';
  }).join('');
}

/* 코스를 누르면 다른 화면으로 넘어가지 않고 그 줄 바로 밑에 레슨이 펼쳐진다.
   한 번에 한 코스만 — 다른 코스를 누르면 앞의 것은 접힌다. 다시 누르면 접힌다. */
let lcOpenId = null;
function clLessonsHtml(c) {
  return `<div class="cl-lessons">${c.lessons.map((l, i) => {
    const done = doneSet.has(l.id);
    return `<button type="button" class="ll-row${done ? ' done' : ''}" data-lesson="${esc(l.id)}" data-c="${esc(c.id)}">` +
...
$('lcList').addEventListener('click', (ev) => {
  ...
  if (l) {
    const c = COURSES.find((x) => x.id === l.dataset.c);
    if (c) startLesson(c, c.lessons.find((x) => x.id === l.dataset.lesson));
    return;
  }
  const b = ev.target.closest('[data-course]');
  if (!b || b.disabled) return;
  const c = COURSES.find((x) => x.id === b.dataset.course);
  if (lcOpenId === c.id) { lcOpenId = null; drawCourses(); return; }
  openCourse(c);
});
...
async function openCourse(c) {
  ...
  lcOpenId = c.id;
  await drawCourses();
  // 펼친 줄이 화면 위쪽에 오게 — 긴 목록 한가운데서 눌러도 레슨이 바로 보인다.
  document.querySelector(`#lcList .cl-row[data-course="${c.id}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
}
...
/* 쓰기는 급(초·중·고)이 아니라 문항 번호로 고른다. 실제 시험에서 쓰기는 TOPIK II
   51~54번 넷뿐이고, 급으로 가르면 51·52번이 「중급」, 53·54번이 「고급」에 들어가
   초급을 누르면 빈 목록이 나왔다. 수험생은 「54번 연습」처럼 번호로 찾는다. */
let twQ = 51;
function twQSwitch() {
  const cur = TW_QS.find((x) => x.q === twQ) || TW_QS[0];
  const cond = (TW_ITEMS.find((x) => x.q === twQ) || {}).cond || '';
  return (
    '<div class="pt-lv-row">' +
      `<div class="diff-seg learn-lv" role="radiogroup" aria-label="${t('문항', 'Question')}">` +
        TW_QS.map((g) =>
          `<label><input type="radio" name="tw_q" value="${g.q}" data-tw-q="${g.q}"${g.q === twQ ? ' checked' : ''}><span>${esc(t(`${g.q}번`, `Q${g.q}`))}</span></label>`
        ).join('') +
      '</div>' +
      `<div class="pt-lv-desc">${esc(t(cur.ko, cur.en))}${cond ? ` · ${esc(cond)}` : ''}</div>` +
    '</div>'
  );
}
function twDrawHead() {
  const mine = TW_ITEMS.filter((x) => x.q === twQ);
  const g = TW_QS.find((x) => x.q === twQ) || {};
  $('twLevel').innerHTML = twQSwitch();
  ...
  $('twSummary').innerHTML = renderLearnSummary([
    { k: t('문항', 'Items'), v: mine.length, s: t('이 번호에서 연습할 문항', 'Practice items for this question') },
    { k: t('배점', 'Points'), v: g.pt || '', s: t('실제 시험의 배점', 'As on the real exam') },
    { k: t('번호', 'Question'), v: twQ, s: t('기출이 아닌 창작 문항', 'Original items, not past papers') },
  ]);
}
...
function twDrawRecord() {
  const box = $('twRecord');
  const mine = TW_ITEMS.filter((x) => x.q === twQ);
  const rows = [twQ].filter((q) => q <= 52).map((q) => {
  ...
  box.innerHTML =
    `<div class="tq-bd-h">${esc(t(`${twQ}번 진행`, `Progress on Q${twQ}`))}</div>` +
...
function twDraw() {
  ...
  twDrawRecord();
  if (!mine.length) {
    $('twList').innerHTML = `<div class="learn-empty">${esc(t('이 단계 쓰기 문항은 아직 채우는 중이에요.', 'Writing items for this level are still being written.'))}</div>`;
    return;
  }
...
function twOpen(it) {
  ...
  $('twDesk').innerHTML =
    ...
    `<button type="button" class="rd-say" data-say="${esc(it.passage)}" data-audio="${AUDIO_BASE}write/${esc(it.id)}.mp3">🔊 ${esc(t('지문 듣기', 'Listen to the passage'))}</button>` +
...
function twNextSameType() {
  const it = twItem;
  if (!it) return;
  const mine = TW_ITEMS.filter((x) => x.q === it.q);
...
async function finishLesson() {
  ...
  /* 코스의 마지막 레슨이면 다음 레슨이 없다. 그런데 존댓말 시리즈처럼
     코스끼리 needs 로 이어진 자리는 여기서 목록으로 돌아가면 학습자가
     스스로 다음 코스를 찾아 눌러야 한다 — 레슨은 이어지는데 코스 경계에서
     끊기는 것이다. 이 코스를 needs 로 가리키는 코스가 있으면 그리로
     바로 잇는다. 없으면(대부분의 코스) 예전처럼 목록으로 돌아간다. */
  const nextCourse = !next ? COURSES.find((c) => c.needs === lsCourse.id) : null;
  const first = !doneSet.has(lsLesson.id);
  // 처음 깬 레슨만 센다 — 같은 레슨을 복습 삼아 몇 번 더 도는 것까지
  // 세면 "얼마나 많은 사람이 첫 레슨까지 가나" 라는 퍼널이 흐려진다.
  if (first) track('레슨완료');
...
$('learnBtn').addEventListener('click', async () => {
  // TOPIK 은 배우기 화면 틀을 빌려 쓰지만 딴 곳이다 — 거기서 누르면 배우기로 간다.
  if ((showing('learnView') && lsecOpen !== 'topik') || showing('lessonView')) return open('home');
  open('learn');
  await loadProgress();
...
/* ══ 레벨 테스트 ═══════════════════════════════════════════════
   #askDlg 와 같은 <dialog> 팝업(#ltDlg)이다 — 화면을 갈아 끼우지
   않고 지금 보던 곳 위에 뜬다.

   열면 먼저 목적을 고른다(전체·TOPIK·읽기·쓰기·듣기). TOPIK 목적만
   기존 TOPIK 읽기 문제(topik.js·topik2.js)를 그대로 쓰고, 나머지 넷
   (전체·읽기·쓰기·듣기)은 leveltest-*.js 에 자체 제작해 둔 문제를
   쓴다 — 각 파일 머리말에 정확한 모양을 적어 뒀다(Gemini 같은 데
   시켜 새로 채울 때 그대로 지시문으로 쓸 수 있다).

   문제는 5개만 낸다 — 쉽게. 자체 제작 자료가 몇 개든 그중 5개를
   무작위로 뽑는다(gameShuffle). 점수(맞힌 개수 / 5)만으로 결과를
   가른다 — 단계별로 따로 세지 않는다.

   설문(목적·학습 시간)은 "전체"를 골랐을 때만 보여 준다 — 다른 목적은
   무엇을 확인하고 싶은지 이미 고른 것과 같으므로 또 묻지 않는다. */
const LT_PURPOSES = [
  { id: 'overall', ko: '전체', en: 'Overall',
    subKo: '문법 감각을 골고루 확인해요', subEn: 'A well-rounded grammar check' },
  { id: 'topik', ko: 'TOPIK', en: 'TOPIK',
    subKo: 'TOPIK 연습을 어디서 시작할지', subEn: 'Where to start TOPIK practice' },
  { id: 'reading', ko: '읽기', en: 'Reading',
    subKo: '어떤 난이도 지문이 맞는지', subEn: 'Which passage level fits you' },
  { id: 'writing', ko: '쓰기', en: 'Writing',
    subKo: '문장을 얼마나 정확히 완성하는지', subEn: 'How accurately you complete sentences' },
  { id: 'listening', ko: '듣기', en: 'Listening',
    subKo: 'TOPIK 듣기를 어디서 시작할지', subEn: 'Where to start TOPIK listening' },
];
const LT_TOTAL = 5;
/* 「전체」 목적의 추천 코스. 점수 구간(sub-tier)마다 시작 코스(start)와
   그 바로 다음 코스(next)를 함께 담는다 — 결과 화면에서 "이 코스 다음엔
   이것"을 함께 보여 주기 위해서다.
   Stage 구조(courses-beginner-stage*.js)에 맞춰 6단계로 세분화한다:
     pre       : 한글 읽기 직후 수준 — 첫 마디·문장 뼈대부터
     beg-early : 초급 1단계(이에요/하다/조사/부정) 시작
     beg-mid   : 초급 3단계(나열·이유·조건·배경) 시작
     beg-late  : 초급 5단계(경험·목적·추측·바람) 시작
     upper     : 중급 시작 */
const LT_RECOMMEND_OVERALL = {
  pre:         { start: 'first-words', next: 'grammar-core' },
  'beg-early': { start: 'bg-05',       next: 'bg-10'        },
  'beg-mid':   { start: 'bg-13',       next: 'bg-17'        },
  'beg-late':  { start: 'bg-22',       next: 'bg-26'        },
  upper:       { start: 'im-02-02',    next: 'im-03-01'     },
};
/* 맞힌 개수(0~5)를 초급 Stage 구조와 맞춘 다섯 구간으로 묶는다.
   0~1개: 아직 기초 전(pre) / 2개: 초급 초반 / 3개: 초급 중반
   4개: 초급 후반 / 5개: 중급 진입(upper) */
const ltBucketFromScore = (score) => (
  score >= LT_TOTAL ? 'upper'
  : score >= 4      ? 'beg-late'
  : score >= 3      ? 'beg-mid'
  : score >= 2      ? 'beg-early'
  : 'pre'
...
/* ── 레벨 8단계 ────────────────────────────────────────────────
   「전체」 테스트는 5문제 무작위로는 레벨을 못 가른다 — 어느 문제가
   뽑히느냐에 따라 같은 사람이 두 단계씩 오르내렸다. 그래서 문제마다
   레벨(lv)을 달고, 맞히면 한 레벨 위 · 틀리면 한 레벨 아래에서 다음
   문제를 내는 계단식으로 10문제까지 묻는다(ltAdaptNext).

   레벨은 코스 짜임(courses-beginner-stage*.js)과 TOPIK 급수에 맞췄다.
   start·next 는 결과 화면이 권하는 코스, tier 는 예전 5구간 이름이다 —
   선생님 말·로드맵·목표별 코스처럼 tier 로 짠 표들을 그대로 쓰려고 둔다. */
const LT_LEVELS = [
  { lv: 0, ko: '한글 전', en: 'Pre-Hangul',   topik: '',          start: 'hangul',      next: 'first-words',  tier: 'pre' },
  { lv: 1, ko: '입문',    en: 'Starter',      topik: '',          start: 'first-words', next: 'grammar-core', tier: 'pre' },
  { lv: 2, ko: '초급 1',  en: 'Beginner 1',   topik: 'TOPIK 1',   start: 'bg-05',       next: 'bg-10',        tier: 'beg-early' },
  { lv: 3, ko: '초급 2',  en: 'Beginner 2',   topik: 'TOPIK 1',   start: 'bg-10',       next: 'bg-13',        tier: 'beg-early' },
  { lv: 4, ko: '초급 3',  en: 'Beginner 3',   topik: 'TOPIK 2',   start: 'bg-13',       next: 'bg-17',        tier: 'beg-mid' },
  { lv: 5, ko: '초급 4',  en: 'Beginner 4',   topik: 'TOPIK 2',   start: 'bg-22',       next: 'bg-27',        tier: 'beg-late' },
  { lv: 6, ko: '중급',    en: 'Intermediate', topik: 'TOPIK 3–4', start: 'im-02-02',    next: 'im-03-01',     tier: 'upper' },
  { lv: 7, ko: '고급',    en: 'Advanced',     topik: 'TOPIK 5–6', start: 'ad-01-01',    next: 'ad-02-01',     tier: 'upper' },
];
const LT_ADAPT_MAX = 10;     // 최대 문항 수
const LT_ADAPT_START = 3;    // 처음 물을 레벨 — 가운데보다 조금 아래서 시작해 겁을 덜 준다
const LT_LEVEL_KEY = 'cp_level';
let ltAd = null;             // 적응형 판의 상태 { cur, used:Set, by:{lv:{c,w}} }

/* 다음에 낼 문제. 지금 레벨(cur)에 남은 문제가 없으면 가까운 레벨에서
   고른다. 판을 끝낼 때면 null.
   일찍 끝내는 경우 — 한 레벨에서 두 번 틀렸고 바로 아래 레벨은 두 번
...
/* ── 내 코스 ──────────────────────────────────────────────────
   레벨테스트 결과(cp_level)와 끝낸 레슨으로 한 사람만의 길을 보여 준다.
   배우기 첫 화면 맨 위에 선다(drawSections → myCourseHtml).
   - 레벨마다 들어갈 코스(초급은 Stage 파일과 needs 순서, 중·고급은 level 로)
   - 「오늘 할 레슨」 — 그 레벨 길에서 아직 안 끝낸 첫 레슨. 레벨을 다 끝냈으면
     다음 레벨의 첫 레슨으로 이어진다.
   - 다시 볼 레슨 — 레슨에서 틀린 빈칸이 있었던 것(cp_weak). 다시 풀어 하나도
     안 틀리면 목록에서 빠진다.
   - 목표에 맞춘 곳 하나(여행·TOPIK·드라마·회화).
   진도는 로그인하면 서버(lesson_progress)에 있지만, 로그인 안 한 사람도 길이
   이어지도록 이 기기에도 적어 둔다(cp_done_local). */
const MY_LEVEL_COURSES = {
  0: ['hangul'],
  1: ['first-words', 'grammar-core'],
  2: ['bg-05', 'bg-06', 'bg-d-01', 'bg-07', 'bg-08', 'bg-09', 'bg-irr-01'],
  3: ['bg-10', 'bg-d-02', 'bg-11', 'bg-12', 'bg-irr-02', 'bg-d-03', 'bg-d-04', 'bg-d-05'],
  4: ['bg-13', 'bg-14', 'bg-15', 'bg-16', 'bg-17', 'bg-18', 'bg-19', 'bg-20', 'bg-21'],
  5: ['bg-22', 'bg-23', 'bg-24', 'bg-25', 'bg-26', 'bg-27', 'bg-04', 'bg-28', 'bg-29', 'bg-30',
      'bg-31', 'bg-32', 'bg-33', 'bg-34', 'bg-35', 'bg-36'],
};
function myLevelCourses(L) {
  const ids = MY_LEVEL_COURSES[L];
  if (ids) return ids.map((id) => COURSES.find((c) => c.id === id)).filter(Boolean);
  const want = L === 6 ? 'Intermediate' : 'Advanced';
  return COURSES.filter((c) => c.level === want && c.lessons.length);
}
/* 함수 선언으로 둔다 — loadProgress 가 모듈이 다 돌기 전에 불려도 쓸 수 있게. */
function myRead(k, d) { try { const v = JSON.parse(localStorage.getItem(k) || 'null'); return v ?? d; } catch (e) { return d; } }
function myWrite(k, v) { try { localStorage.setItem(k, JSON.stringify(v)); } catch (e) {} }
function myMarkDone(lessonId, courseId, wrong) {
  const done = new Set(myRead('cp_done_local', []));
  done.add(lessonId);
  myWrite('cp_done_local', [...done]);
...
function myNext(L) {
  for (let k = L; k < LT_LEVELS.length; k++) {
    for (const c of myLevelCourses(k)) for (const l of c.lessons) if (!doneSet.has(l.id)) return { c, l, lv: k };
  }
  return null;
}
const MY_GOAL_EXTRA = {
  travel:  { emoji: '✈️', ko: '여행 한국어',   en: 'Travel Korean',         go: () => 'travel' },
...
function myAct(my) {
  ...
  else if (act === 'lesson' && c) { track('내코스레슨'); startLesson(c, c.lessons.find((l) => l.id === my.dataset.l) || c.lessons[0]); }
  else if (act === 'course' && c) { lsCourse = c; learnLv.courses = courseTier(c); backToCourses(); }
...
async function drawMine() {
  ...
  let html = myCourseHtml();
  if (me) {
    const L = me.lv;
    const row = (c) => {
      const n = courseDone(c), all = c.lessons.length;
      return `<button type="button" class="my-row${n >= all ? ' done' : ''}" data-my="course" data-c="${esc(c.id)}">` +
        `<span class="my-row-e">${courseIcon(c.emoji)}</span>` +
        `<span class="my-row-t">${esc(cTx(c.title))}</span>` +
        `<span class="my-row-n">${n >= all ? '✓' : `${n} / ${all}`}</span></button>`;
    };
    html += `<div class="my-sec my-sec-lg">${esc(t(`L${L} · ${ltLevelName(L)} 코스`, `L${L} · ${ltLevelName(L)} courses`))}</div>` +
      `<div class="my-rows">${myLevelCourses(L).map(row).join('')}</div>`;
    if (L < LT_LEVELS.length - 1) {
      const nx = myLevelCourses(L + 1).slice(0, 3);
      if (nx.length) html += `<div class="my-sec my-sec-lg">${esc(t(`다음 레벨 L${L + 1} · ${ltLevelName(L + 1)}`, `Next: L${L + 1} · ${ltLevelName(L + 1)}`))}</div>` +
        `<div class="my-rows soon">${nx.map(row).join('')}</div>`;
    }
  }
  /* 레벨테스트를 안 한 사람에게도 코스가 보여야 한다 — 안내 카드 한 장만 있으면
     「코스 섹션」이 비어 보인다. 레벨마다 코스를 늘어놓고, 스스로 고를 수 있게 한다. */
  if (!me) {
    const row = (c) => `<button type="button" class="my-row" data-my="course" data-c="${esc(c.id)}">` +
      `<span class="my-row-e">${courseIcon(c.emoji)}</span>` +
      `<span class="my-row-t">${esc(cTx(c.title))}</span>` +
      `<span class="my-row-n">${c.lessons.length}</span></button>`;
    html += `<div class="my-sec my-sec-lg">${esc(t('레벨을 알면 바로 골라도 돼요', 'Know your level? Pick it'))}</div>`;
    for (const x of LT_LEVELS) {
      const list = myLevelCourses(x.lv);
      if (!list.length) continue;
      const more = list.length - 4;
      html += `<div class="my-lv-h"><span>L${x.lv} · ${esc(ltLevelName(x.lv))}${x.topik ? ` <small>${esc(x.topik)}</small>` : ''}</span>` +
        `<button type="button" class="my-link" data-my="pick" data-lv="${x.lv}">${esc(t('이 레벨로 시작', 'Start here'))}</button></div>` +
        `<div class="my-rows">${list.slice(0, 4).map(row).join('')}` +
        (more > 0 ? `<button type="button" class="my-row more" data-my="pick" data-lv="${x.lv}"><span class="my-row-t">${esc(t(`코스 ${more}개 더 — 이 레벨로 시작하면 다 보여요`, `${more} more — start here to see them all`))}</span></button>` : '') +
        '</div>';
    }
  }
  $('myBody').innerHTML = html;
}
$('myWrap').addEventListener('click', (ev) => {
  const my = ev.target.closest('[data-my]');
  if (my) myAct(my);
});
function myCourseHtml() {
  const me = ltLoadLevel();
  if (!me) {
    return '<div class="my-card my-empty">' +
  ...
  if (nx) {
    const up = nx.lv > L;
    today = `<button type="button" class="my-next" data-my="lesson" data-c="${esc(nx.c.id)}" data-l="${esc(nx.l.id)}">` +
      `<span class="my-next-k">${esc(up ? t(`다음 레벨 L${nx.lv} 첫 레슨`, `First lesson of L${nx.lv}`) : t('오늘 할 레슨', "Today's lesson"))}</span>` +
      `<span class="my-next-t">${esc(cTx(nx.l.title))}</span>` +
      `<span class="my-next-c">${esc(cTx(nx.c.title))} →</span></button>`;
  } else {
  ...
  const weak = myRead('cp_weak', []).map((w) => {
    ...
    return c && l ? { c, l, n: w.n } : null;
  }).filter(Boolean).slice(0, 3);
  const extra = me.goal && MY_GOAL_EXTRA[me.goal];
  return '<div class="my-card">' +
    '<div class="my-head">' +
      '<div>' +
        `<div class="my-kicker">${esc(t('내 코스', 'MY COURSE'))}</div>` +
        `<div class="my-title">L${L} · ${esc(ltLevelName(L))}${info.topik ? ` <small>${esc(info.topik)}</small>` : ''}</div>` +
      '</div>' +
    ...
    (weak.length
      ? `<div class="my-sec">${esc(t('다시 볼 레슨', 'Worth another look'))}</div><div class="my-chips">` +
        ...
        '</div>' : '') +
    (extra
      ? `<div class="my-sec">${esc(t('내 목표에 맞춰', 'For your goal'))}</div>` +
        `<button type="button" class="my-chip wide" data-my="go" data-go="${esc(extra.go(L))}">${esc(extra.emoji)} ${esc(t(extra.ko, extra.en))} →</button>` : '') +
    `<button type="button" class="my-link block" data-my="open">${esc(t(`내 코스 · L${L} 전체 보기 →`, `My course · all of L${L} →`))}</button>` +
  '</div>';
}
...
const LT_TIER_MSG = {
  pre:         { ko: '아직 기초 단계예요. 첫 마디부터 차근차근 쌓아 봐요.', en: "You're at the very beginning — let's build the basics step by step." },
  'beg-early': { ko: '초급 1단계 수준이에요. 이에요/예요부터 탄탄히 다져 봐요.', en: 'You\'re at Beginner Stage 1. Let\'s build from the essentials.' },
  'beg-mid':   { ko: '초급 3단계 수준이에요. 이유·조건 표현으로 문장 폭을 넓혀 봐요.', en: 'You\'re at Beginner Stage 3. Let\'s expand with reasons and conditions.' },
  'beg-late':  { ko: '초급 5단계 수준이에요. 경험·추측 표현까지 한 단계 더!', en: 'You\'re at Beginner Stage 5 — almost to intermediate level!' },
  upper:       { ko: '중급 수준이에요! 뉘앙스 차이를 더 정교하게 다져 봐요.', en: 'You\'re at intermediate level! Let\'s refine the nuances.' },
};
...
/* 전체·읽기·쓰기·듣기 — 자체 제작 문제(leveltest-*.js). 파일 하나당
   내보내는 이름은 LT_CUSTOM_<PURPOSE 대문자>. 지연 로딩은 다른 자료
   (sbNeed 등)와 같은 짜임이다. */
/* 파일마다 따로 불러온다(문자열을 이어 붙여 한 곳에서 부르지 않는다) —
   tools/stamp.mjs 가 자국(?v=)을 박으려면 import 자리마다 파일 이름이
   그대로 적혀 있어야 한다. 이어 붙이면 그 줄에서 이름을 못 찾아 자국이
...
const ltCustomOverallNeed = () => (ltCustomOverallP ??= import('./leveltest-overall.js?v=d0eca8e9').then((m) => { LT_CUSTOM.overall = m.LT_CUSTOM_OVERALL; }));
const ltCustomReadingNeed = () => (ltCustomReadingP ??= import('./leveltest-reading.js?v=d0eca8e9').then((m) => { LT_CUSTOM.reading = m.LT_CUSTOM_READING; }));
const ltCustomWritingNeed = () => (ltCustomWritingP ??= import('./leveltest-writing.js?v=d0eca8e9').then((m) => { LT_CUSTOM.writing = m.LT_CUSTOM_WRITING; }));
const ltCustomListeningNeed = () => (ltCustomListeningP ??= import('./leveltest-listening.js?v=d0eca8e9').then((m) => { LT_CUSTOM.listening = m.LT_CUSTOM_LISTENING; }));
...
const ltCustomNeed = (purpose) => LT_CUSTOM_NEED[purpose]();

/* 자체 제작 문제 뽑기. passage 가 있으면(읽기) 지문+물음을 물음으로
   합친다. 듣기는 raw 에 원본을 남겨 둔다 — tlSpeak 가 script 를 그걸로
   읽는다. */
async function ltMakeCustomRound(purpose) {
  await ltCustomNeed(purpose);
  return gameShuffle(LT_CUSTOM[purpose]).slice(0, LT_TOTAL).map((item) => ({
    ask: item.passage ? `${item.passage}\n\n${item.q}` : item.q,
    answer: item.options[item.answer],
    options: gameShuffle(item.options.slice()),
    raw: purpose === 'listening' ? item : undefined,
  }));
}

/* TOPIK 목적만 기존 TOPIK 읽기 문제(topik.js·topik2.js, tqNeedData 가
   불러온다)를 그대로 재활용한다 — 여기만 자체 제작이 아니다. */
async function ltMakeTopikRound() {
  await tqNeedData();
  const pool = [...TQ_EXAMS.I.reading, ...TQ_EXAMS.II.reading];
  return gameShuffle(pool).slice(0, LT_TOTAL).map((q) => ({
    ask: q.passage ? `${q.passage}\n\n${q.question}` : q.question,
    answer: q.options[q.answer],
    options: gameShuffle(q.options.slice()),
  }));
}

/* 목적별 문제 만들기. 다 { ask, answer, options, raw? } 모양의 배열을
...
function ltDraw() {
  ...
  $('ltAsk').classList.toggle('lt-ask-multiline', ltPurpose === 'topik' || ltPurpose === 'reading');
  $('ltMsg').textContent = '';
  ...
  q.options.forEach((opt, oIdx) => {
    ...
    numSpan.className = 'qz-opt-num';
    numSpan.textContent = optNums[oIdx] || String(oIdx + 1);
    const txtSpan = document.createElement('span');
    txtSpan.className = 'qz-opt-text';
    txtSpan.textContent = opt;
    b.append(numSpan, txtSpan);
    b.addEventListener('click', () => ltPick(opt, b, q));
    box.appendChild(b);
  });
...
function ltPick(opt, btn, q) {
  ...
  if (right) {
    ...
    btn.classList.add('wrong');
    all.forEach((b) => {
      const txt = b.querySelector('.qz-opt-text')?.textContent || b.textContent;
      if (txt === q.answer) b.classList.add('right');
    });
    $('ltMsg').className = 'claw-msg bad';
    $('ltMsg').textContent = t(`「${q.answer}」예요.`, `It's “${q.answer}”.`);
  }
  ltIdx++;
  ltMeta();
  setTimeout(() => {
    // 그새 팝업을 닫았으면 그만둔다 — 안 보이는 곳에서 판이 저 혼자 끝난다.
...
async function ltSetRecommendation(purpose, tier, goal, level) {
  if (purpose === 'overall') {
    await coursesNeed();
    const goalMod = goal ? LT_GOAL_COURSE_MOD[goal]?.[tier] : null;
    /* 레벨이 있으면 레벨 표가 이긴다 — 5구간 표는 L2·L3 을 한데 묶어서
       같은 코스를 권했다. 목표별 코스(goalMod)는 그대로 우선한다. */
    const rec = goalMod || (level != null ? LT_LEVELS[level] : LT_RECOMMEND_OVERALL[tier]);
    ...
    ltRecommendation = !course ? null : {
      emoji: course.emoji, lv: courseLabel(course),
      title: cTx(course.title), tagline: cTx(course.tagline),
      cta: t('이 코스로 시작하기 →', 'Start with this course →'),
      action: () => { lsCourse = course; learnLv.courses = courseTier(course); backToCourses(); },
    ...
    ltRecommendation = {
      emoji: '✍️', lv: t('TOPIK II · 쓰기', 'TOPIK II · Writing'),
      title: t('TOPIK 쓰기 연습', 'TOPIK writing practice'),
      tagline: t('문장 완성부터 짧은 글쓰기까지', 'From sentence completion to short essays'),
      cta: t('여기서 시작하기 →', 'Start here →'),
      action: () => { open('learn'); openLearnSub('topik/II/writing'); },
      next: null,
    };
  } else {
    // topik · reading · listening — pre/beg-* 구간은 TOPIK I, upper는 II.
    await tqNeedData();
    const exam = (tier === 'pre' || tier === 'beg-early' || tier === 'beg-mid') ? 'I' : 'II';
    const skill = purpose === 'listening' ? 'listening' : 'reading';
    const ex = TQ_EXAMS[exam];
    const examName = t(ex.name.ko, ex.name.en);
    const skillName = t(TQ_SKILLS[skill].name.ko, TQ_SKILLS[skill].name.en);
    ltRecommendation = {
      emoji: TQ_SKILLS[skill].emoji, lv: `${examName} · ${skillName}`,
      title: t(`${skillName} 연습`, `${skillName} practice`), tagline: t(ex.sub.ko, ex.sub.en),
      cta: t('여기서 시작하기 →', 'Start here →'),
  ...
  $('ltRecTitle').textContent = ltRecommendation.title;
  $('ltRecTagline').textContent = ltRecommendation.tagline;
  $('ltRecCta').textContent = ltRecommendation.cta;

  // ── 다음 코스 힌트 (전체 목적 + 다음 코스가 있을 때만) ─────
  if (ltRecommendation.next) {
    const nxt = ltRecommendation.next;
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'lt-rec-next';
    const lbl = document.createElement('span');
    lbl.className = 'lt-rec-next-label';
    lbl.textContent = t('다음', 'Next');
    const ttl = document.createElement('span');
    ttl.className = 'lt-rec-next-title';
    ttl.textContent = `${nxt.emoji ? nxt.emoji + ' ' : ''}${nxt.title}`;
    const arr = document.createElement('span');
    arr.className = 'lt-rec-next-arrow';
    arr.textContent = '→';
    btn.append(lbl, ttl, arr);
    btn.addEventListener('click', () => {
      $('ltDlg').close();
      nxt.action();
    });
...
function ltSeeAllAction(purpose) {
  open('learn');
  if (purpose === 'overall') openSection('courses');
  else openLearnSub('topik');
}
$('ltRecCard').addEventListener('click', () => {
  if (!ltRecommendation) return;
  $('ltDlg').close();
  track('추천코스클릭');
  ltRecommendation.action();
});
$('ltSeeAll').addEventListener('click', () => { $('ltDlg').close(); track('전체목록클릭'); ltSeeAllAction(ltPurpose); });
$('ltReplay').addEventListener('click', () => {
  const q = ltRound[ltIdx];
  if (q?.raw) tlSpeak(q.raw);
});
...
/* ── 레벨테스트 결과 공유 ─────────────────────────────────────
   학습자는 서로 레벨을 견주고 싶어 한다. 결과를 정사각형 이미지로 구워 폰의 공유 창에
   넘긴다(인스타 스토리 · 카톡 · 디스코드에 그대로 붙는다). 링크는 레벨테스트가 바로
   열리는 주소 — 받은 사람이 「나는 몇이지?」를 3분 안에 해 볼 수 있게.
   이미지를 공유 못 하는 브라우저(대부분의 PC)는 이미지를 내려받고 글과 링크를 복사한다. */
let ltLast = null;
const LT_SHARE_URL = 'https://everykoreans.com/?lt=1&utm_source=share&utm_medium=level_card';
async function ltShareCard(L) {
  const W = 1080, c = document.createElement('canvas');
  c.width = W; c.height = W;
  const g = c.getContext('2d');
  const FONT = "-apple-system, BlinkMacSystemFont, 'Apple SD Gothic Neo', 'Noto Sans KR', 'Malgun Gothic', sans-serif";
  g.fillStyle = '#F2EEE4'; g.fillRect(0, 0, W, W);
  g.fillStyle = '#FFFFFF';
  g.beginPath(); g.roundRect ? g.roundRect(70, 70, W - 140, W - 140, 56) : g.rect(70, 70, W - 140, W - 140); g.fill();
  try {
    const img = new Image(); img.src = 'logo-256.png';
    await img.decode();
  ...
  // 8칸 길 — 내 자리만 주황
  for (let i = 0; i < 8; i++) {
    g.fillStyle = i < L.level ? '#FFD9B8' : i === L.level ? '#FF914D' : '#EFE8DC';
    g.beginPath(); const x = 140 + i * 102; g.roundRect ? g.roundRect(x, 780, 88, 20, 10) : g.rect(x, 780, 88, 20); g.fill();
  }
  g.fillStyle = '#1B1512'; g.font = `700 50px ${FONT}`; g.fillText(t('나는 몇 단계일까? 🧀🥔', 'What level are you? 🧀🥔'), 140, 890);
...
$('ltShare').addEventListener('click', async () => {
  ...
  try {
    ...
    const file = blob && new File([blob], `cheesepotato-L${L.level}.png`, { type: 'image/png' });
    if (file && navigator.canShare && navigator.canShare({ files: [file] })) {
      await navigator.share({ files: [file], text: `${text} ${LT_SHARE_URL}` });
      return;
    }
    if (navigator.share) { await navigator.share({ text, url: LT_SHARE_URL }); return; }
    // PC: 이미지는 내려받고, 글과 링크는 복사
    if (blob) {
      const a = document.createElement('a');
...
try {
  ...
  /* 공유된 레벨 카드의 링크는 /?lt=1 로 온다 — 레벨테스트를 바로 연다. */
  if (q.has('lt')) {
    q.delete('lt');
...
/* ══ 홈 화면에 설치 (PWA) ════════════════════════════════════
   폰 홈 화면에 아이콘이 생기면 다시 오기가 훨씬 쉽다. 처음 온 날은 조르지 않는다 —
   다른 날 두 번째로 왔을 때부터. 닫으면 30일 안 띄운다. 이미 설치해서 앱으로 열었으면
   (display-mode: standalone) 안 띄운다. 「다시 공부할 것」 띠(nudgeBar)가 떠 있는 날은
   그쪽을 먼저 — 띠 두 개가 겹치면 둘 다 안 읽힌다.
   · 안드로이드 · PC 크롬/엣지: 브라우저가 주는 설치 창(beforeinstallprompt)을 연다.
   · 아이폰 · 아이패드 사파리: 설치 창이 없어서 「공유 → 홈 화면에 추가」를 글로 안내한다. */
(() => {
  const X_KEY = 'cp_install_x', DAYS_KEY = 'cp_visit_days';
  let deferred = null;
  const standalone = matchMedia('(display-mode: standalone)').matches || navigator.standalone === true;
  const ua = navigator.userAgent;
  const ios = /iPhone|iPad|iPod/.test(ua) || (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
  const iosSafari = ios && /Safari/.test(ua) && !/CriOS|FxiOS|EdgiOS|FBAN|FBAV|Instagram|KAKAOTALK|Line/.test(ua);
  let days = 1;
  try {
    const today = new Date().toISOString().slice(0, 10);
    const d = JSON.parse(localStorage.getItem(DAYS_KEY) || '[]');
    if (!d.includes(today)) d.push(today);
    localStorage.setItem(DAYS_KEY, JSON.stringify(d.slice(-10)));
    days = d.length;
  } catch (e) {}
  const snoozed = () => { try { return Date.now() - Number(localStorage.getItem(X_KEY) || 0) < 30 * 864e5; } catch (e) { return false; } };
  function show(kind) {
    if (standalone || days < 2 || snoozed() || !$('nudgeBar').hidden) return;
    $('installTxt').textContent = kind === 'ios'
      ? t('홈 화면에 추가하면 앱처럼 바로 열려요 — 아래 공유 버튼 ⬆️ → 「홈 화면에 추가」', 'Add to your Home Screen to open it like an app — tap Share ⬆️ → “Add to Home Screen”')
      : t('치즈감자를 홈 화면에 두고 앱처럼 열어 보세요', 'Put CheesePotato on your home screen and open it like an app');
    $('installGo').hidden = kind === 'ios';
    $('installGo').textContent = t('설치', 'Install');
    $('installBar').hidden = false;
    track('설치안내봄');
  }
  addEventListener('beforeinstallprompt', (ev) => {
    ev.preventDefault();           // 브라우저가 멋대로 띄우지 않게 — 우리 띠에서 연다
    deferred = ev;
    setTimeout(() => show('prompt'), 4000);   // 첫 화면이 다 뜬 뒤에
  });
  if (iosSafari) setTimeout(() => show('ios'), 4000);
  $('installGo').addEventListener('click', async () => {
    if (!deferred) return;
    $('installBar').hidden = true;
    deferred.prompt();
    const { outcome } = await deferred.userChoice.catch(() => ({ outcome: 'dismissed' }));
    track(outcome === 'accepted' ? '설치함' : '설치안함');
    deferred = null;
  });
  $('installX').addEventListener('click', () => {
    $('installBar').hidden = true;
    try { localStorage.setItem(X_KEY, String(Date.now())); } catch (e) {}
  });
...
var EPS = null;
var epsP = null;
function epsNeed() { return (epsP ??= import('./eps.js?v=d0eca8e9').then((m) => { EPS = m; })); }
...
/* 녹음소(record.html)에서 녹음한 파일(assets/audio/eps/<id>.mp3)이 있으면 그것을,
   없으면 브라우저 목소리로. tlAudioEl 에 걸어 두어 tlStop 이 같이 끊는다. */
function epsSay(it) {
  tlStop();
  let fell = false;
  const fall = () => { if (fell) return; fell = true; tlAudioEl = null; tlWebSpeech(epsLines(it), () => {}); };
  try {
    const a = new Audio(`${AUDIO_BASE}eps/${it.id}.mp3`);
    tlAudioEl = a;
    a.onended = () => { tlAudioEl = null; };
    a.onerror = fall;
    a.play().catch(fall);
  } catch (e) { fall(); }
}
