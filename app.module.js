...
async function openLearnSub(sub) {
  ...
  /* learn/eps/<문항 id> — eps-topik/ 정적 쪽의 「직접 풀어보기」. 그 문항의 유형 연습을
     ...
     처음 화면을 그리고, 그 뒤에 여기서 연습을 시작한다. */
  if (secId === 'eps' && rest[0]) {
    openSection('eps', true);
    await epsNeed();
    const it = EPS.EPS_ITEMS.find((x) => x.id === rest[0]);
    if (it) {
      const list = epsByType(it.sec, it.type);
      epsStart('practice', { sec: it.sec, type: it.type, at: list.indexOf(it) });
    }
    return;
  }
...
async function epsDraw() {
  ...
  host.innerHTML =
    ...
    `<p class="eps-note">${t(
      ...
      'Original practice questions that follow the real format and level — not past papers, and not affiliated with HRD Korea. Pass marks differ by country and round.')}` +
