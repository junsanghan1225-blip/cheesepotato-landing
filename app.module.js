...
import { createClient } from './vendor/supabase-js.js?v=db70bdc5';
...
let XLSX = null;
const needXLSX = async () => (XLSX ??= await import('./vendor/xlsx.js?v=db70bdc5'));
// 커리큘럼. 내용과 엔진을 갈라 두면 글을 고치다 화면을 깨지 않는다.
import { COURSES } from './courses.js?v=db70bdc5';
import { GLOSSARY, GLOSS_LANGS } from './glossary.js?v=db70bdc5';
import { glossFind } from './gloss-find.js?v=db70bdc5';
import { GRAMMAR } from './grammar.js?v=db70bdc5';
import { GRAMMAR_EN } from './grammar-en.js?v=db70bdc5';
import { grammarScan } from './grammar-find.js?v=db70bdc5';
import { TW_ITEMS, TW_QS } from './topik-writing.js?v=db70bdc5';
import { TOPIKL_BY_EXAM, TOPIKL_PICTURE_SLOTS } from './topik-listening.js?v=db70bdc5';
import { SB_CATS, SB_MORE, SB_SEED } from './sentences.js?v=db70bdc5';
// 읽기 연습 지문. 길이(short·long) × 급수 여섯 칸.
// TOPIK 유형 연습문제. 기출이 아니라 자체 제작이다.
// 숫자 게임의 읽기와 문제 만들기. 화면을 모르는 순수 계산이라 따로 뒀다.
import { makeRound } from './numbers.js?v=db70bdc5';
...
const tqNeedData = () => (tqDataP ??= Promise.all([
  import('./topik.js?v=db70bdc5'), import('./topik2.js?v=db70bdc5'),
]).then(([a, b]) => {
...
const rdNeed = () => (rdP ??= import('./reading.js?v=db70bdc5').then((m) => { READING = m.READING; }));
...
const dictLoadSenses = () => (dictSensesP ??=
  import('./glossary-senses.js?v=db70bdc5').then((m) => m.SENSES).catch(() => ({})));
...
const dictLoadExamples = () => (dictExamplesP ??=
  import('./glossary-examples.js?v=db70bdc5').then((m) => m.EXAMPLES).catch(() => ({})));
