...
import { createClient } from './vendor/supabase-js.js?v=14636b02';
// TOPIK 읽기 "문제 풀이 영상" 목록. 아주 작은 파일이라(id 목록뿐) 다른
// 자료처럼 갈래를 열 때 지연 로딩하지 않고 그냥 처음부터 받는다.
import { TQ_VIDEO_IDS } from './topik-video.js?v=14636b02';
...
let XLSX = null;
const needXLSX = async () => (XLSX ??= await import('./vendor/xlsx.js?v=14636b02'));
// 커리큘럼. 내용과 엔진을 갈라 두면 글을 고치다 화면을 깨지 않는다.
// 갈래 목록(drawSections)·코스(drawCourses)·문제만 풀기(dqDraw) 를 열 때만
// 받는다 — 배우기 갈래 목록도 안 본 사람에게 코스 71개 레슨을 다 물릴
// 까닭이 없다. warmLearn() 이 배우기를 여는 순간 미리 불을 붙여 둔다.
let COURSES = [], coursesP = null;
const coursesNeed = () => (coursesP ??= import('./courses.js?v=14636b02').then((m) => { COURSES = m.COURSES; }));
...
const glossNeed = () => (glossP ??= import('./glossary.js?v=14636b02').then((m) => {
...
const grammarNeed = () => (grammarP ??= Promise.all([
  import('./grammar.js?v=14636b02'), import('./grammar-en.js?v=14636b02'),
]).then(([a, b]) => { GRAMMAR = a.GRAMMAR; GRAMMAR_EN = b.GRAMMAR_EN; }));
import { grammarScan } from './grammar-find.js?v=14636b02';
...
const sbNeed = () => (sbDataP ??= import('./sentences.js?v=14636b02').then((m) => {
...
let makeRound = null;
const needNumbers = async () => (makeRound ??= (await import('./numbers.js?v=14636b02')).makeRound);
...
const tqNeedData = () => (tqDataP ??= Promise.all([
  import('./topik.js?v=14636b02'), import('./topik2.js?v=14636b02'),
  import('./topik-writing.js?v=14636b02'), import('./topik-listening.js?v=14636b02'),
]).then(([a, b, c, d]) => {
...
const rdNeed = () => (rdP ??= Promise.all([
  import('./reading.js?v=14636b02'), grammarNeed(),
]).then(([m]) => { READING = m.READING; }));

let CONVO = null, cvP = null;
const cvNeed = () => (cvP ??= import('./convo.js?v=14636b02').then((m) => { CONVO = m.CONVO; }));
...
const dictLoadSenses = () => (dictSensesP ??=
  import('./glossary-senses.js?v=14636b02').then((m) => m.SENSES).catch(() => ({})));
...
const dictLoadExamples = () => (dictExamplesP ??=
  import('./glossary-examples.js?v=14636b02').then((m) => m.EXAMPLES).catch(() => ({})));
...
let tvP = null;
const tvNeed = () => (tvP ??= import('./travel-data.js?v=14636b02').then((m) => {
...
const ltCustomOverallNeed = () => (ltCustomOverallP ??= import('./leveltest-overall.js?v=14636b02').then((m) => { LT_CUSTOM.overall = m.LT_CUSTOM_OVERALL; }));
const ltCustomReadingNeed = () => (ltCustomReadingP ??= import('./leveltest-reading.js?v=14636b02').then((m) => { LT_CUSTOM.reading = m.LT_CUSTOM_READING; }));
const ltCustomWritingNeed = () => (ltCustomWritingP ??= import('./leveltest-writing.js?v=14636b02').then((m) => { LT_CUSTOM.writing = m.LT_CUSTOM_WRITING; }));
const ltCustomListeningNeed = () => (ltCustomListeningP ??= import('./leveltest-listening.js?v=14636b02').then((m) => { LT_CUSTOM.listening = m.LT_CUSTOM_LISTENING; }));
