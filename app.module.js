...
import { createClient } from './vendor/supabase-js.js?v=4a2cdd67';
...
let XLSX = null;
const needXLSX = async () => (XLSX ??= await import('./vendor/xlsx.js?v=4a2cdd67'));
// 커리큘럼. 내용과 엔진을 갈라 두면 글을 고치다 화면을 깨지 않는다.
// 갈래 목록(drawSections)·코스(drawCourses)·문제만 풀기(dqDraw) 를 열 때만
// 받는다 — 배우기 갈래 목록도 안 본 사람에게 코스 71개 레슨을 다 물릴
// 까닭이 없다. warmLearn() 이 배우기를 여는 순간 미리 불을 붙여 둔다.
let COURSES = [], coursesP = null;
const coursesNeed = () => (coursesP ??= import('./courses.js?v=4a2cdd67').then((m) => { COURSES = m.COURSES; }));
import { GLOSSARY, GLOSS_LANGS } from './glossary.js?v=4a2cdd67';
import { glossFind } from './gloss-find.js?v=4a2cdd67';
/* 문법 사전(뜻풀이 197개). 읽기 지문의 밑줄 문법 말풍선(rdNeed)과 예문
   만들기 화면(sbNeed) 양쪽이 쓴다 — 둘 중 먼저 여는 화면이 받아 두고,
   나중 화면은 그 약속(??=)을 그대로 쓴다. */
let GRAMMAR = [], GRAMMAR_EN = {}, grammarP = null;
const grammarNeed = () => (grammarP ??= Promise.all([
  import('./grammar.js?v=4a2cdd67'), import('./grammar-en.js?v=4a2cdd67'),
]).then(([a, b]) => { GRAMMAR = a.GRAMMAR; GRAMMAR_EN = b.GRAMMAR_EN; }));
import { grammarScan } from './grammar-find.js?v=4a2cdd67';
...
const sbNeed = () => (sbDataP ??= import('./sentences.js?v=4a2cdd67').then((m) => {
...
let makeRound = null;
const needNumbers = async () => (makeRound ??= (await import('./numbers.js?v=4a2cdd67')).makeRound);
...
const tqNeedData = () => (tqDataP ??= Promise.all([
  import('./topik.js?v=4a2cdd67'), import('./topik2.js?v=4a2cdd67'),
  import('./topik-writing.js?v=4a2cdd67'), import('./topik-listening.js?v=4a2cdd67'),
]).then(([a, b, c, d]) => {
...
const rdNeed = () => (rdP ??= Promise.all([
  import('./reading.js?v=4a2cdd67'), grammarNeed(),
]).then(([m]) => { READING = m.READING; }));

let CONVO = null, cvP = null;
const cvNeed = () => (cvP ??= import('./convo.js?v=4a2cdd67').then((m) => { CONVO = m.CONVO; }));
...
const dictLoadSenses = () => (dictSensesP ??=
  import('./glossary-senses.js?v=4a2cdd67').then((m) => m.SENSES).catch(() => ({})));
...
const dictLoadExamples = () => (dictExamplesP ??=
  import('./glossary-examples.js?v=4a2cdd67').then((m) => m.EXAMPLES).catch(() => ({})));
