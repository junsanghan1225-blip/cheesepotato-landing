...
### 동형어 번호를 살리는 길 — 굽는 코드와 화면은 고쳐 두었다

**남은 것은 자료를 다시 굽는 일뿐이다.** 국립국어원 내려받기 폴더가 있어야
한다(1GB, 저장소에 안 들어간다).

```bash
node tools/build-krdict-glossary.mjs <국립국어원_내려받은_폴더>
...
| 자리 | 하는 일 |
|---|---|
| `build-krdict-glossary.mjs` | 「눈01」의 번호를 **열쇠에 함께 넣어** 눈(眼) 과 눈(雪) 을 따로 담는다. 번호가 표제어에 붙어 오든 `sup_no` 칸에 따로 있든 둘 다 본다. 품사도 낱말마다 따로 남는다 |
| `build-glossary-senses.mjs` | 뜻줄에 **몇 번째 낱말인지와 그 품사**를 적어 넘긴다 — `[뜻풀이, 영어, 번호, 품사]`. 안 갈린 표제어는 칸이 둘 그대로다 |
| `build-glossary.mjs` | 카드에 쓸 **낱말별 한 줄 뜻**을 따로 싣는다 — `hom: [[영어, 품사], …]` |
| 화면(`app.module.js`) | 갈린 것만 낱말별로 묶어 그린다. 「글자는 같지만 서로 다른 낱말이에요」를 얹고, 사전처럼 어깨번호를 단다 — 눈¹ · 눈² |

카드의 한 줄 뜻도 갈라 그린다.

```
전            눈   eye; snow
갈라 그린 뒤   눈¹ eye · 눈² snow
```

**번호는 사전 차례 그대로 두고, 영어 뜻이 없는 낱말은 카드에서 빼되 번호는
건너뛴다.** 「차」의 접사 次 가 그렇다 — 카드에 「차² car · 차³ tea」로 나온다.
빼고 번호를 다시 매기면 「더 보기」의 차² 와 카드의 차² 가 서로 다른 낱말을
가리키게 된다.

**지금 자료로 돌려도 아무것도 안 바뀐다.** 동형어 번호가 없으니 덩이가 하나뿐이고,
그러면 뜻줄 칸이 둘 그대로이며 화면도 예전처럼 한 목록으로 그린다. 다시 구운
자료가 들어오는 순간 저절로 갈라진다.

굽는 코드는 원본 1GB 없이는 못 돌려 보므로, 국립국어원 자료 모양을 흉내 낸
자료로 확인했다 — 번호가 표제어에 붙은 「눈01·눈02」와 `sup_no` 칸에 든
「차 01·02·03」이 각각 따로 담기고 품사가 각자 남는 것까지 봤다.
화면은 갈린 자료를 임시로 넣고 브라우저로 그려 봤다.
...
원고지는 투명한 `<textarea>` 가 통째로 덮고 있고, 보이는 칸은 그 값을 옮겨
그린 것이다. 그래서 **칸을 눌러도 클릭이 전부 textarea 로 갔다.** textarea 의
줄바꿈은 픽셀 폭이 정하고 원고지의 줄바꿈은 칸 수가 정하니 둘이 다르고,
엉뚱한 자리에 커서가 섰다. 위아래 화살표도 마찬가지라 결국 좌우 화살표로
한 칸씩 밀고 가는 수밖에 없었다.

이제 칸마다 「여기를 누르면 커서가 몇 번째 글자로 가는지」를 함께 들고 있다.

- `.tw-grid-in` 에 `pointer-events:none` — 클릭을 원고지가 받는다
- 빈 칸에도 갈 자리가 있다. 들여쓰기 칸은 그 문단 첫 글자, 줄 끝의 남은
  칸은 그 줄 끝, 글이 끝난 뒤의 칸은 글 맨 끝
- 위아래 화살표는 **칸 기준**으로 옮긴다(`twMoveRow`). 좌우·글자·IME 는
  그대로 textarea 가 한다 — 한글 조합과 되돌리기를 잃으면 안 된다

### 53·54 번 채점

51·52 번은 빈칸이라 답을 맞춰 보면 점수가 나온다. 53·54 번은 글이라 여태
점수가 없었다. 이제 「채점하기」가 채점표를 낸다.

**내용은 채점하지 않는다.** 「좋은 습관이 삶에 미치는 영향을 제대로 썼는가」
는 뜻을 읽어야 아는 일이다. 낱말이 겹치는지 세어서 숫자를 낼 수는 있지만
그것은 **점수처럼 보이는 숫자일 뿐이고**, 학습자는 그 숫자를 믿는다. 그래서
세부 과제를 목록으로 내주고 사람이 짚게 한다.

기계가 재는 것은 자료의 `deduct`(감점 사유)에 적힌 것과 그대로 겹친다.
사람이 채점해도 제일 먼저 보는 것들이라 그렇다.

| | 재는 것 |
|---|---|
| 분량 | 200자(53) · 600자(54) 를 못 채우면 감점. 자료가 「가장 흔한 감점」이라 적어 둔 것 |
| 문체 | -(느)ㄴ다체에 해요체가 섞였는가 (`twRegisterMiss` 가 이미 하던 일) |
| 전개 | 54 번은 서론·본론·결론으로 문단이 나뉘는가 |
| 표현 | 이어 주는 표현을 **몇 가지** 썼는가. 「그래서」를 열 번 써도 한 가지다 |

배점은 공개된 TOPIK II 쓰기 기준을 따랐다(`TW_RUBRIC`). 바뀌면 그 표만 고치면 된다.

**53 번과 54 번은 재는 것이 다르다.** 처음에 둘 다 「문단을 나눴는가」로
쟀다가 자료의 모범답안으로 시험해 보고서야 알았다 — 53 번 상(29점) 답안이
문단 하나짜리라 감점을 맞았다. 자료 설명 글은 원래 한 문단으로 쓴다.
자료의 `points.structure` 도 문단이 아니라 「차례로 이어지고 연결 표지를
썼는가」라고 적혀 있었다. **규칙을 짓기 전에 자료에 뭐라고 적혀 있는지부터
읽었어야 했다.**

자료 설명 표현 목록도 머릿속으로 적었다가 다시 뽑았다. 「그 뒤를 이었다」
같은 긴 꼴만 적어 두어서, 「1위를 차지하였고 · 2위에 올랐다 · 이어」로 쓴
진짜 답안이 하나도 안 걸렸다.

자료에 실린 모범답안으로 재 보면 이렇다.

| | 사람이 매긴 점수 | 기계 |
|---|---|---|
| 54 번 상 · 중 · 하 | 47 · 32 · 16 | 50 · 28 · 18 |
| 53 번 상 · 중 · 하 | 29 · 19 · 9 | 28 · 21 · 18 |

내용 칸을 다 짚었다고 보고 잰 값이다. 53 번 「하」가 높은 것은 그 때문이다 —
실제로는 세부 과제를 다 못 짚는다.
