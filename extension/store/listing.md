...
> **올리기 전에 확인할 것.** 이 쪽은 안드로이드 단어장 앱의 방침을 구운
> 것이다(`tools/build-privacy.js`, 원본은 앱 저장소의
> `docs/privacy-policy.md`). 익스텐션 이야기가 한 줄도 없으면 심사에서
> 걸린다. 「이 익스텐션은 아무것도 모으지 않고 바깥으로 보내지 않는다」는
> 절을 원본에 더하고 다시 구울 것.
