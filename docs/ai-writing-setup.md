...
1. **SQL Editor** 에서 `db/add_writing_usage.sql` 을 통째로 돌린다(하루 사용 횟수 표 `writing_usage`).
   ※ `ai_usage` 라는 표는 발음 진단 · 한국어 도우미가 이미 쓰는 것이라 따로 만든다.
...
4. 사이트에서 로그인 → TOPIK → TOPIK II → 쓰기 → 54번 문항 → 50자 넘게 쓰고 「🤖 AI 채점」.
   ...
   - 500: `writing_usage` 표나 `is_pro` 함수가 없다(1번)
